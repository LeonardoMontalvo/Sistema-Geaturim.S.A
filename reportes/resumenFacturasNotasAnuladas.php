...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("RESUMEN DE VENTAS ANULADAS"), 0, 1, 'C', 0);
        ...
        $this->Cell(24, 6, utf8_decode('Fecha'), 1, 0, 'C', 1);
        ...
        $this->Cell(24, 6, utf8_decode('Anulación'), 1, 0, 'C', 1);
...
$consulta1 = pg_query(
    "SELECT num_factura,fv.fecha_cancelacion,hora_actual,fecha_anulacion,tipo_precio,forma_pago,tarifa0,tarifa12,iva_venta,descuento_venta,
    total_venta,identificacion,nombres_cli,nombre_empresa,id_factura_venta 
    from factura_venta fv, clientes c, empresa e, usuario u where fv.id_cliente=c.id_cliente  
    and u.id_usuario=fv.id_usuario and fv.id_empresa='$_GET[id]' 
    and fv.fecha_actual $query_fecha '$_GET[fin]' and fv.estado='Pasivo' order by fv.id_factura_venta asc"
);
if (pg_num_rows($consulta1)) {
    while ($row1 = pg_fetch_row($consulta1)) {
        ...
        $pdf->Cell(16, 6, number_format($row1[10] - $row1[8] + $row1[9], 2, '.', ''), 0, 0, 'R', 0);
        $desc = $desc + $row1[9];
        $pdf->Cell(16, 6, number_format($row1[9], 2, '.', ''), 0, 0, 'R', 0);
        $pdf->Cell(16, 6, number_format($row1[6], 2, '.', ''), 0, 0, 'R', 0);
        $pdf->Cell(16, 6, number_format($row1[7], 2, '.', ''), 0, 0, 'R', 0);
        $ivaT = $ivaT + $row1[8];
        $pdf->Cell(16, 6, number_format($row1[8], 2, '.', ''), 0, 0, 'R', 0);
        $total = $total + $row1[10];
        $t0 = $row1[6];
        $pdf->Cell(16, 6, number_format($row1[10], 2, '.', ''), 0, 0, 'R', 0);
        $pdf->Cell(24, 6, $row1[3], 0, 0, 'C', 0);
    ...
    $pdf->Cell(16, 6, maxCaracter((number_format($sub - $desc, 2, ',', '.')), 20), 0, 0, 'R', 0);
