...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("RESUMEN DE NOTAS DE COMPRA"), 0, 1, 'C', 0);
...
if (pg_num_rows($consulta)) {
    while ($row = pg_fetch_row($consulta)) {
        ...
        if (pg_num_rows($consulta1)) {
            while ($row1 = pg_fetch_row($consulta1)) {
                ...
                $pdf->Cell(38, 6, utf8_decode(substr($row[3], 0, 19)), 0, 0, 'C', 0);
                ...
                $desc = $desc + $row1[9];
                $pdf->Cell(12, 6, utf8_decode(truncateFloat(round($row1[9], 2, PHP_ROUND_HALF_EVEN), 2)), 0, 0, 'R', 0);
    ...
    $pdf->Cell(185, 6, utf8_decode("Iva Total"), 0, 0, 'R', 0);
