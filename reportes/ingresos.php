...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("REPORTE DE INGRESOS POR TRANSFERENCIA"), 0, 1, 'C', 0);
...
$util = 0;
$consulta = pg_query("
select i.id_ingresos, 
i.fecha_actual, i.origen, 
i.destino,tarifa0, 
i.tarifa12, 
i.iva_ingreso, 
i.descuento_ingreso, 
...
where i.fecha_actual $query_fecha '$_GET[fin]' and i.estado='Activo'
and i.id_empresa=$idpuntov");
while ($row = pg_fetch_assoc($consulta)) {
    $repetido = 0;
    $sql1 = pg_query("select d.id_ingresos, p.articulo, d.cantidad, d.precio_costo, d.descuento, d.total from detalle_ingreso d, productos p where d.id_ingresos=$row[id_ingresos] and d.cod_productos=p.cod_productos  and d.estado='Activo'");
    if (pg_num_rows($sql1)) {
        if ($repetido == 0) {
        ...
        //        $pdf->Cell(20, 6, (number_format($row["iva_ingreso"], 2, ',', '.')), 0, 0, 'C', 0);
        //        $pdf->Ln(5);
        //        $pdf->Cell(180, 6, utf8_decode('Descuento: '), 0, 0, 'R', 0);
        //        $pdf->Cell(20, 6, (number_format($row["descuento_ingreso"], 2, ',', '.')), 0, 0, 'C', 0);
        $pdf->Ln(5);
        $pdf->Cell(180, 6, utf8_decode('Total: '), 0, 0, 'R', 0);
        $pdf->Cell(20, 6, (number_format($row["total_ingreso"], 2, ',', '.')), 0, 0, 'C', 0);
        $total = 0;
    }
    $pdf->Ln(6);
}
