...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(150, 5, "VENTAS PRODUCTOS", 0, 1, 'R', 0);
        $this->SetFont('Arial', 'B', 16);
        $this->SetX(0);
        $this->Cell(210, 8, "EMPRESA: " . $_SESSION['empresa'], 0, 1, 'C', 0);
        $this->Image('../images/logo_empresa.jpg', 5, 8, 35, 28);
        $this->SetFont('Amble-Regular', '', 10);
        $this->SetX(0);
        $this->Cell(190, 5, "PROPIETARIO: " . utf8_decode($_SESSION['propietario']), 0, 1, 'C', 0);
        $this->SetX(0);
        $this->Cell(105, 5, "TEL.: " . utf8_decode($_SESSION['telefono']), 0, 0, 'R', 0);
        $this->Cell(105, 5, "CEL.: " . utf8_decode($_SESSION['celular']), 0, 1, 'L', 0);
        $this->SetX(0);
...
class Reporte
{
    ...
    public function titulo($titulo)
    {
        ...
        $this->pdf->Line(1, 50, 210, 50);
        $this->pdf->SetFont('Arial', 'B', 12);
        $this->pdf->SetX(0);
        $this->pdf->Cell(210, 5, utf8_decode($_GET['inicio']) . " - " . utf8_decode($_GET['fin']), 0, 1, 'C', 0);
        $this->pdf->SetX(0);
        $this->pdf->Cell(210, 5, utf8_decode($titulo), 0, 1, 'C', 0);
    ...
    public function tabla($tipo)
    {
        ...
        $this->pdf->SetAligns(["C", "C", "C", "C", "C"]);
        $this->pdf->Row(["#", "Nombre Producto",  "Cantidad", "Total"], 1);

        switch ($tipo) {
            case "venta":
                $sql = "
        select x.cod_productos,x.articulo, sum(x.cantidad) as cantidad, sum(x.total) as total, x.iva from(
        (select 
        dfv.cod_productos, 
        ...
        *100)/nullif((fv.tarifa0::numeric+fv.tarifa12::numeric),0),0)/100))),4),0) as total, 
        ...
        group by dfv.cod_productos, p.articulo, p.iva
        ...
        *100)/nullif((fv.tarifa0::numeric+fv.tarifa12::numeric),0),0)/100))),4),0) as total, 
        ...
        group by x.cod_productos, x.articulo, x.iva
        order by cantidad desc
        ...
        foreach ($rows as $key => $value) {
            ...
            $this->pdf->Row([$key + 1, utf8_decode($value["articulo"]),  $value["cantidad"], number_format($totali, 2, ",", ".")], 1);
            $total += $totali;
            $cantidad += $value["cantidad"];
        }
        $this->pdf->SetFont("Arial", "B", 10);
        $this->pdf->SetWidths([$totalw - ($colw * 2) + 25, $colw - 20, $colw - 20]);
        $this->pdf->SetAligns(["R", "l", "l"]);
        $this->pdf->Row(["Totales", $cantidad, number_format($total, 2, ",", ".")]);
    ...
    public function imprimir($titulo, $tipo)
    {
        $this->titulo($titulo);
        $this->tabla($tipo);
        $this->pdf->Output();
...
switch ($_GET["tipo"]) {
    case "venta":
        $repo->imprimir("RESUMEN DE PRODUCTOS VENDIDOS", $_GET["tipo"]);
        break;
    case "reservacion":
        $repo->imprimir("RESUMEN DE PRODUCTOS RESERVADOS", $_GET["tipo"]);
        break;
    case "credito":
        $repo->imprimir("RESUMEN DE PRODUCTOS CRÉDITO", $_GET["tipo"]);
        break;
}
