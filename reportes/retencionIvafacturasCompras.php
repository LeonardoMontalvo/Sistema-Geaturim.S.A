...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(190, 5, utf8_decode("RESUMEN DE RETENCIONES DE IVA DE FACTURAS COMPRAS"), 0, 1, 'C', 0);
...
if (pg_num_rows($consulta1)) {
    ...
    while ($row1 = pg_fetch_row($consulta1)) {
        ...
        $pdf->Cell(80, 6, utf8_decode(substr($row1[1], 0, 50)), 0, 0, 'C', 0);
        $pdf->Cell(22, 6, utf8_decode($row1[2]), 0, 0, 'C', 0);
        $pdf->Cell(22, 6, number_format($row1[3], 2, ',', '.'), 0, 0, 'R', 0);
        $pdf->Cell(22, 6, number_format($row1[4], 2, ',', '.'), 0, 0, 'R', 0);
        $pdf->Cell(22, 6, number_format($row1[5], 2, ',', '.'), 0, 1, 'R', 0);
        $total += $row1[3];
        $base += $row1[4];
        $ret += $row1[5];
    }
