        ...
        while ($row = pg_fetch_row($sql)) {
            $sql1 = pg_query(
                ...
                SELECT cp.num_factura,
                    ...
                    fecha_actual,
                    fecha_emicion,
                    (fecha_vencimiento::date-fecha_emicion::date),
                    '0',
                    '0',
                    total,
            ...
            if (pg_num_rows($sql1)) {
                ...
                while ($row1 = pg_fetch_row($sql1)) {
                    ...
                    $pdf->Cell(26, 6, number_format($row1[9], 2, ',', '.'), 0, 0, 'R', 0);
                    $pdf->Cell(30, 6, "E", 0, 1, 'R', 0);
                    $subta += $row1[5];
                    $subto += $row1[7];
                    $subabo += $row1[8];
                    $subsa += $row1[9];
        ...
        while ($row = pg_fetch_row($sql)) {
            ...
            (
                SELECT g.num_factura,
                    ...
                    (fpm.fecha_actual::date - fecha_credito::date)dias_vence,
                    adelanto,
                    meses,
                    monto_credito,
...
function obtenerCpIternasExternas($idproveedor)
{
    ...
    (
        SELECT cp.num_factura num_doc,
            ...
            (fecha_vencimiento::date - fecha_emicion::date) dias_caduca,
            '0' adelanto,
            '0' meses,
            total::numeric,
            (total::numeric - saldo::numeric) as abonos,
