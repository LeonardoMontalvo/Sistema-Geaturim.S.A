...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("PRODUCTOS"), 0, 1, 'C', 0);
        $this->Ln(7);
        $this->SetX(0);
        ...
        $this->Cell(87, 6, utf8_decode(""), 1, 0, 'C', 1);
...
$sql = "select cod_barras,codigo,articulo
from productos p
...
if (pg_num_rows($consulta)) {
    while ($row = pg_fetch_assoc($consulta)) {
        ...
        $pdf->Cell(33, 6, maxCaracter(utf8_decode($row["cod_barras"]), 20), "B", 0, 'L', 0);
        $pdf->Cell(70, 6, maxCaracter(utf8_decode($row["articulo"]), 20), "B", 0, 'L', 0);

        $pdf->Cell(86, 6, "", "B", 0, 'R', 0);
