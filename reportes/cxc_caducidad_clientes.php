...
$cliente = pg_fetch_assoc(pg_query("SELECT id_cliente, identificacion, nombres_cli FROM clientes WHERE id_cliente=$_GET[id]"));
$pdf->Cell(208, 8, utf8_decode("CLIENTE:  $cliente[nombres_cli]  CI:  $cliente[identificacion]"), 1, 0, 'C', true);
$pdf->Ln(9); 
$pdf->SetX(1);
...
if ($_GET['tipo'] == 'Externas') {
    ...
    $query = pg_query(
        "SELECT id_c_cobrarexternas, comprobante, fecha_actual, fecha_emision, 
        case when (fecha_emision::date - fecha_actual::date)>=0 
        and (fecha_dias::date - fecha_credito::date)<=30 then saldo else 0 end,
        case when (fecha_emision::date - fecha_actual::date)>30 
        and (fecha_dias::date - fecha_credito::date)<=60 then saldo else 0 end, 
        case when (fecha_emision::date - fecha_actual::date)>60 
        and (fecha_dias::date - fecha_credito::date)<=90 then saldo else 0 end,
        case when (fecha_emision::date - fecha_actual::date)>90 
        and (fecha_dias::date - fecha_credito::date)<=120 then saldo else 0 end, 
        case when (fecha_emision::date - fecha_actual::date)>120 then saldo else 0 end, saldo
        FROM c_cobrarexternas inner join clientes c using(id_cliente)
        where c.id_cliente=$cliente[id_cliente] and  c_cobrarexternas.id_empresa='$_SESSION[PV]' order by id_c_cobrarexternas"
    );
    ...
    $query = pg_query(
        "SELECT id_pagos_venta, num_factura, pv.fecha_credito, pv.fecha_dias, 
        ...
        inner join factura_venta fv using(id_factura_venta) 
        where c.id_cliente=$cliente[id_cliente] and  fv.id_empresa='$_SESSION[PV]' order by pv.id_pagos_venta"
    );
    if (pg_num_rows($query)) {
        $sub = 0;
        while ($row = pg_fetch_row($query)) {
