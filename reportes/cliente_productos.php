...
");
 ...
 $sql2 = pg_query(" select p.codigo, p.articulo, fv.precio_venta, fv.cantidad 
                    from detalle_factura_venta fv,productos p where fv.cod_productos=p.cod_productos
                    and id_factura_venta='$row1[2]'  and 'VENTAS'='$row1[7]' $id_producto_consult      
union
...
and id_facturas_novalidas='$row1[2]' and fv.estado='Activo' and 'NV'='$row1[7]'
$id_producto_consult ");
...
//                    $pdf->SetFont('helvetica', 'B', 9);
                    ...
                    $pdf->Cell(30, 6, utf8_decode($row[1]), 1, 0, 'L', true);
                      ...
                      $pdf->Cell(33, 6, utf8_decode("F. NRO.: " . $row1[0]), 1, 0, 'L', true);
                $pdf->Cell(33, 6, utf8_decode("FECHA: " . $row1[1]), 1, 0, 'L', true);
                $pdf->Cell(33, 6, utf8_decode("HORA: " . $row1[6]), 1, 0, 'L', true);
                $pdf->Cell(16, 6, utf8_decode($row1[3]), 1, 0, 'L', true);
 ...
 where factura_compra.fecha_emision $query_fecha '$_GET[fin]'
  ...
  and factura_compra.id_proveedor=proveedores.id_proveedor   
    ...
    if (pg_num_rows($sql1)) {
     ...
     where fv.cod_productos=p.cod_productos 
     and id_factura_venta='$row1[2]'  and 'VENTAS'='$row1[7]'
                  $id_producto_consult            
...
where fv.cod_productos=p.cod_productos 
and id_factura_compra='$row1[2]' and 'COMPRA'='$row1[7]'
$id_producto_consult 
                  ");
            ...
            if (pg_num_rows($sql2)) {
                ...
                $pdf->SetFillColor(216, 216, 231);
                $pdf->SetFont('helvetica', 'B', 9);
                $pdf->Cell(50, 6, utf8_decode($row1[5]), 1, 0, 'L', true);
                $pdf->Cell(26, 6, utf8_decode($row1[4]), 1, 0, 'L', true);
                $pdf->Cell(33, 6, utf8_decode("F. NRO.: " . $row1[0]), 1, 0, 'L', true);
                $pdf->Cell(33, 6, utf8_decode("FECHA: " . $row1[1]), 1, 0, 'L', true);
                $pdf->Cell(33, 6, utf8_decode("HORA: " . $row1[6]), 1, 0, 'L', true);
                $pdf->Cell(16, 6, utf8_decode($row1[3]), 1, 0, 'L', true);
