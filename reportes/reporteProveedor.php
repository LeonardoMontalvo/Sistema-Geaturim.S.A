...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 6, utf8_decode("LISTA DE PRODUCTOS POR PROVEEDORES"), 0, 1, 'C', 0);
        $this->SetFont('helvetica', 'B', 10);
        $row = pg_fetch_row(
            pg_query(
                "SELECT PR.empresa_pro FROM productos P INNER JOIN proveedores PR ON PR.id_proveedor = P.id_proveedor 
                WHERE PR.id_proveedor=$_GET[id];"
            )
        );
        $this->Cell(210, 7, utf8_decode("PROVEEDOR: " . $row[0]), 0, 1, 'C', 0);
...
$consulta = pg_query(
    "SELECT codigo, articulo, iva_minorista, iva_mayorista, iva_negocio, stock FROM productos P WHERE P.id_proveedor='$_GET[id]';"
);
...
if (pg_num_rows($consulta)) {
    while ($row = pg_fetch_row($consulta)) {
        ...
        $pdf->Cell(39, 5, maxCaracter(utf8_decode($row[0]), 50), 0, 0, 'L', 0);
