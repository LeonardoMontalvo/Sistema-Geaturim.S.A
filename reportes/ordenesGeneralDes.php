<?php
    ...
    class PDF extends FPDF {   
        ...
        function Header() {                         
            ...
            $this->Cell(190, 5, utf8_decode("RESUMEN DE ORDENES DE PRODUCCION NO APROBADAS"),0,1, 'C',0);                                                                                                                            
            $this->SetFont('Amble-Regular','',10);        
            $this->Ln(3);
    ...
    $no=pg_query("select id_orden from desaprobacion_ordenes where fecha_actual between '$_GET[inicio]' and '$_GET[fin]'");
    while ($des=pg_fetch_row($no)) {
    
        $consulta=pg_query("select id_ordenes, id_receta, fecha_actual, cantidad, estado, costo_total from ordenes_produccion where id_ordenes=$des[0]");
