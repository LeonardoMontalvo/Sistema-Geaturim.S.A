...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(210, 5, utf8_decode("COBROS A: " . maxCaracter($row[2], 30)), 0, 1, 'C', 0);
        ...
        $this->Cell(75, 6, maxCaracter(utf8_decode('RUC/CI:' . $row[2]), 35), 0, 0, 'C', 1);
        $this->Cell(135, 6, maxCaracter(utf8_decode('NOMBRES:' . $row[3]), 50), 0, 1, 'C', 1);
...
$pdf = new PDF('P', 'mm', 'a4');
$pdf->SetTitle('Cobros por Cliente');
...
if ($_GET['id_empre'] != '0') {
    $query_punto = "AND pagos_cobrar.id_empresa='$_GET[id_empre]'";
}
if ($_GET['id'] != '0') {
...
$sql = pg_query(
    ...
    FROM pagos_cobrar WHERE id_cliente='$_GET[id]' and estado='Activo' and tipo_pago='$cuenta' $query_punto $id_usuario
    AND fecha_actual $query_fecha '$_GET[fin]' and tipo_factura='$tipo';"
);
if (pg_num_rows($sql)) {
    $sub = 0;
    $suba = 0;
