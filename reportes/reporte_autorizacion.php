...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $row = pg_fetch_row(pg_query("select identificacion,nombres_cli,telefono,direccion_cli from clientes where id_cliente='$_GET[id]'"));
        $this->Cell(210, 5, utf8_decode("AUTORIZACIONES ENTREGADAS A " . maxCaracter($row[1], 25)), 0, 1, 'C', 0);
        $this->Ln(8);
        $this->SetFillColor(220, 240, 210);
        $this->SetFont('helvetica', 'B', 8.5);
        $this->Cell(40, 6, utf8_decode('RUC/CI: ' . $row[0]), 0, 0, 'C', 1);
        $this->Cell(65, 6, utf8_decode('NOMBRE: ' . maxCaracter($row[1], 25)), 0, 0, 'C', 1);
        $this->Cell(40, 6, utf8_decode('TELF.: ' . $row[2]), 0, 0, 'C', 1);
        $this->Cell(65, 6, utf8_decode('DIR.:  ' . maxCaracter($row[3], 25)), 0, 1, 'C', 1);
        $this->SetX(5);
...
$sql1 = pg_query(
    ...
    FROM factura_venta where id_cliente='$_GET[id]' and estado='Activo';"
);
if (pg_num_rows($sql1)) {
