...
function generarXMLRETGASTO($id, $codDoc, $ambiente, $emision) {

    $consulta = pg_query("SELECT nombre_empresa, ruc_empresa, direccion_empresa, obligacion, establecimiento,
        punto_emision, g.id_gastos, comprobante, fecha_emision, fecha as fecha_aut, nombre_comercial,
        g.num_serie as sec_doc, rffc.num_serie, rffc.clave, rffc.num_autorizacion, 
    ...
    while ($row = pg_fetch_assoc($consulta)) {
        ...
        $fechaEmision_act = $row['fecha_emision'];
...
function generarXMLCDATAFACGASTOS($data) {
    $s = "";
    $s = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
    $s .= "<autorizacion>\n";
