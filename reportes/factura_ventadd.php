<?php
    ...
    /////////medio
    $pdf->SetFont('Amble-Regular','',10);       
    $pdf->Text(10, 48, maxCaracter(utf8_decode($cliente),80),1,0, 'L',0);/////cliente
    $pdf->Text(10, 54, maxCaracter(utf8_decode($ci_ruc),20),1,0, 'L',0);////ruc ci
    $pdf->Text(10, 60, maxCaracter(utf8_decode($direccion),50),1,0, 'L',0);////direccion
    $pdf->Text(113, 55, maxCaracter(utf8_decode($telefono),20),1,0, 'L',0);////telefono
    $pdf->Text(103, 48, maxCaracter(utf8_decode($fecha),20),1,0, 'L',0);/////fecha

    $pdf->Text(170, 48, maxCaracter(utf8_decode($cliente),80),1,0, 'L',0);/////cliente
    $pdf->Text(170, 54, maxCaracter(utf8_decode($ci_ruc),20),1,0, 'L',0);////ruc ci
	$pdf->Text(170, 62, maxCaracter(utf8_decode($direccion),50),1,0, 'L',0);////direccion
    $pdf->Text(268, 55, maxCaracter(utf8_decode($telefono),20),1,0, 'L',0);////telefono
    $pdf->Text(260, 48, maxCaracter(utf8_decode($fecha),20),1,0, 'L',0);/////fecha
 ...
 if($result1=="."||$result2=="."||$result3=="."||$result4=="."||$result5=="."){
       $pdf->Text(115, 205, maxCaracter($total_venta,10),0,1, 'L',0);   
       $pdf->Text(271, 205, maxCaracter($total_venta,10),0,1, 'L',0);    

}
else{
    
      $total_ventacero=$total_venta.".00";
      $pdf->Text(115, 205, maxCaracter($total_ventacero,10),0,1, 'L',0); 
      $pdf->Text(271, 205, maxCaracter($total_ventacero,10),0,1, 'L',0);    
}
     
    $pdf->Text(115, 174, maxCaracter($subtotal,8),0,1, 'L',0);    
    $pdf->Text(115, 182, maxCaracter(utf8_decode($descuento_venta),8),0,1, 'L',0);    
    $pdf->Text(115, 191, maxCaracter(utf8_decode($iva0),8),0,1, 'L',0);
    $pdf->Text(115, 197, maxCaracter(utf8_decode($iva_venta),8),0,1, 'L',0);    
   
    
    $pdf->Text(271, 174, maxCaracter($subtotal,8),2,1, 'L',2);    
    $pdf->Text(271, 182, maxCaracter(utf8_decode($descuento_venta),8),0,1, 'L',0);    
    $pdf->Text(271, 191, maxCaracter(utf8_decode($iva0),8),0,1, 'L',0);
    $pdf->Text(271, 197, maxCaracter(utf8_decode($iva_venta),8),0,1, 'L',0);    
