...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(35, 6, utf8_decode("FACTURA"), 1, 0, 'C', 1);
        $this->Cell(30, 6, utf8_decode("CODIGO"), 1, 0, 'C', 1);
        $this->Cell(85, 6, utf8_decode("PRODUCTO"), 1, 0, 'C', 1);
        $this->Cell(20, 6, utf8_decode("P. COMPRA"), 1, 0, 'C', 1);
        $this->Cell(20, 6, utf8_decode("T. COMPRA"), 1, 0, 'C', 1);
        $this->Cell(20, 6, utf8_decode("CANTIDAD"), 1, 1, 'C', 1);
    }
    function Footer()
    {
        $this->SetY(-15);
...
$total = 0;
$sql = pg_query(
    "SELECT proveedores.id_proveedor, identificacion_pro,factura_compra.id_factura_compra,num_serie 
    FROM proveedores,factura_compra where proveedores.id_proveedor='$_GET[id]' and factura_compra.id_proveedor=proveedores.id_proveedor;"
);
if (pg_num_rows($sql)) {
    while ($row = pg_fetch_row($sql)) {
        ...
        while ($row1 = pg_fetch_row($sql1)) {
            ...
            $pdf->Cell(34, 5, maxCaracter(utf8_decode($row[3]), 20), 0, 0, 'L', 0);
            $pdf->Cell(30, 5, maxCaracter(utf8_decode($row1[1]), 12), 0, 0, 'L', 0);
            $pdf->Cell(85, 5, maxCaracter(utf8_decode($row1[2]), 40), 0, 0, 'L', 0);
            $pdf->Cell(20, 5, number_format($row1[6], 2, ',', '.'), 0, 0, 'R', 0);
            $pdf->Cell(20, 5, number_format($row1[7], 2, ',', '.'), 0, 0, 'R', 0);
            $pdf->Cell(20, 5, number_format($row1[8], 2, ',', '.'), 0, 1, 'R', 0);
            $total += $row1[7];
        }
    }
    $pdf->SetX(1);
    ...
    $pdf->Cell(175, 5, "Totales:", 0, 0, 'R', 0);
