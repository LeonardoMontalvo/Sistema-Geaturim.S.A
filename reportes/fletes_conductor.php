...
$fechaf = $_GET['fin'];
$condicion = "";
$titulo = "LISTA FLETES TODOS LOS VEHÍCULOS";
$dni = "";
$sql = "";
if ($_GET['idc'] != "") {
    $placa = mb_strtoupper(trim($_GET['idc']));
    if (!empty($dni)) {
        $titulo = "LISTA FLETES TRANSPORTISTA $dni";
        $condicion = "and cc.id_conductor='$dni'";
    }
    $sql = "SELECT cc.dni,c.nombres_cli,cr.ruta_completa,c.identificacion,cat.*
        FROM contrato_conductor cc
        inner join flete_alquiler_vehiculo cav
        on cc.id_conductor=cav.id_conductor
     ...
     <header>
        <table class="header">
            <tr>
                ...
                <img src="../images/<?php echo $_SESSION["parametros_empresa"]["logo_empresa"]?>" alt="" width="70">
                </td>
        ...
        <?php foreach ($contratos as $key => $val) { ?>
            ...
            <table>
                <tr style="background-color: #c1d5e0;">
                    ...
                    <td colspan="3"><b>CONDUCTOR:</b> <?php echo $val['dni'] ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="3"><b>FECHA FELTE:</b> <?php echo $val['fecha_contrato'] ?></td>
                    <td colspan="3"><b>VALOR FELTE:</b> <?php echo $val['valor'] ?></td>
                </tr>
                <tr>
...
$dompdf->render();
$dompdf->stream('reporte_fletes_vehiculos' . '.pdf', array('Attachment' => 0));
?>