...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("REPORTE DE RETENCIONES IVA DE GASTOS"), 0, 1, 'C', 0);
...
$pdf->AddPage();
$pdf->AliasNbPages();
$pdf->SetTitle('REPORTE DE RETENCIONES IVA DE GASTOS');
...
$sub_t = 0;
$codigo_reten = '';
$consulta = pg_query('select * from retencion_iva order by id_retencion_iva asc');
while ($row = pg_fetch_row($consulta)) {
    ...
    $sql1 = pg_query("select fc.num_factura, p.empresa_pro, rfc.fecha, rfc.valor_compra, rf.valor, rfc.valor_retencion,p.identificacion_pro,rf.codigo_formulario 
    from gastos fc, 
    retencion_iva rf, 
    retencion_fuente_factura_compra rfc
    inner join detallecomprobanteretencion dcr
    on rfc.id_retencion_fuente_factura_compra=dcr.id_retencion_fuente_factura_compra
    and dcr.id_trete=2, 
    proveedores p 
    where fc.id_gastos=rfc.id_factura 
    and fc.estado='Activo'
    and fc.id_proveedor=p.id_proveedor 
    and rfc.id_retencion_fuente=rf.id_retencion_iva
    ...
    if (pg_num_rows($sql1) > 0) {
        while ($row1 = pg_fetch_row($sql1)) {
            if ($repetido == 0) {
                ...
                $pdf->Cell(70, 6, (('RETENCION:' . $row[4] . '        -       ' . $row1[4] . '%')), 1, 0, 'L', 1);
            ...
            $pdf->Cell(25, 6, utf8_decode(round($row1[5],2)), 0, 1, 'C', 0);
...
$consulta1 = pg_query('select * from retencion_iva order by id_retencion_iva asc');
while ($row = pg_fetch_row($consulta1)) {
    ...
    $sql1 = pg_query("select fc.num_factura,
    p.empresa_pro,
    rfc.fecha,
    rfc.valor_compra,
    rf.valor,
    ...
    rf.codigo_formulario from gastos fc,
   retencion_iva rf,
   retencion_fuente_factura_compra rfc
   inner join detallecomprobanteretencion dcr
   on rfc.id_retencion_fuente_factura_compra=dcr.id_retencion_fuente_factura_compra
   and dcr.id_trete=2, 
   proveedores p where fc.id_gastos=rfc.id_factura 
   and fc.id_proveedor=p.id_proveedor 
   and rfc.id_retencion_fuente=rf.id_retencion_iva 
   and rfc.id_retencion_fuente='$row[0]' 
   and fc.fecha_emision between '$_GET[inicio]' 
   ...
   order by rfc.id_retencion_fuente_factura_compra");
    if (pg_num_rows($sql1) > 0) {
        ...
        $pdf->Cell(25, 6, round($sub,2), 0, 1, 'C', 0);
    }
}
$pdf->SetX(1);
