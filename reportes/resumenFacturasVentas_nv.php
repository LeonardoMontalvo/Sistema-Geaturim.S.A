...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(210, 5, utf8_decode('NOTAS DE VENTA POR CLIENTE'), 0, 1, 'C', 0);
...
if ($pdf->rango) {
   ...
   if ($_GET['id1'] != '') {
//            $this->cliente = true;
          $query = pg_query("select * from clientes where id_cliente={$_GET['id1']} order by id_cliente asc");
        } else {
          $query = pg_query("select * from clientes order by id_cliente asc");
        }

while ($row = pg_fetch_row($query)) {
    $consulta1 = pg_query("select FV.comprobante, FV.fecha_actual, FV.hora_actual, FV.fecha_actual, tipo_precio, forma_pago, tarifa0, tarifa12, iva_venta, descuento_venta, total_venta,
 identificacion, nombres_cli, nombre_empresa, id_facturas_novalidas, FV.estado, U.nombre_usuario, U.nombre_usuario FROM facturas_novalidas FV 
 INNER JOIN clientes C ON C.id_cliente = FV.id_cliente INNER JOIN empresa E ON E.id_empresa = FV.id_empresa 
 INNER JOIN usuario U ON U.id_usuario = FV.id_usuario WHERE FV.id_empresa='$_GET[id]' and FV.id_cliente='{$row[0]}'
  and FV.fecha_actual $query_fecha '$_GET[fin]' order by FV.id_facturas_novalidas asc
