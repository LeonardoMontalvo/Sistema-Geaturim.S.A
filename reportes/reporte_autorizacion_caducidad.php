...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(105, 5, utf8_decode($fecha), 0, 0, 'C', 0);
        $this->Cell(105, 5, utf8_decode($_GET['fin']), 0, 1, 'C', 0);
        $this->Ln(3);
        $this->SetX(5);
...
$pdf->AddPage();
$pdf->SetTitle('Autorizaciones a Caducar');
$pdf->AliasNbPages();
...
if (pg_num_rows($consulta)) {
    while ($row = pg_fetch_row($consulta)) {
        $sql1 = pg_query(
            ...
            FROM factura_venta where id_cliente='$row[0]' and estado='Activo' and fecha_caducidad between '$fecha' and '$_GET[fin]'"
        );
        if (pg_num_rows($sql1)) {
            $pdf->SetX(0);
