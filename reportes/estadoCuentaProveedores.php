...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode('ESTADOS DEL PROVEEDOR'), 0, 1, 'C', 0);
        ...
        $row = pg_fetch_row(pg_query("select * from proveedores where id_proveedor='$_GET[id]' order by id_proveedor asc"));
...
$consulta1 = pg_query(
    ...
    from c_pagarexternas c INNER JOIN tipo_comprobante t ON c.tipo_documento=t.id_tipo_comprobante 
    where id_proveedor='$_GET[id]' 
    and fecha_actual $query_fecha '$_GET[fin]' 
    order by id_c_pagarexternas asc"
);
...
$consulta2 = pg_query(
    "SELECT tipo_documento, num_serie, fecha_credito, fecha_caducidad, monto_credito, (monto_credito::numeric - saldo::numeric) as abonos, saldo, cp.estado 
    FROM pagos_compra cp 
    LEFT JOIN factura_compra fc USING(id_factura_compra) 
    WHERE cp.id_proveedor='$_GET[id]'
