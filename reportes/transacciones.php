...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $total1 = 0;
        $concepto = "";
        $consulta1 = pg_query("select  T.fecha_actual, T.hora_actual, U.nombre_usuario, U.apellido_usuario, T.num_transaccion, T.concepto, T.total_debe, T.total_haber, T.saldo, TT.descripcion, T.id_transacciones, TT.abreviatura,T.comprobante from transacciones T, usuario U, tipo_transaccion TT where T.id_usuario=U.id_usuario and T.id_transacciones='$_GET[id]' and T.id_tipo_transaccion=TT.id_tipo_transaccion");
...
T.comprobante from transacciones T, usuario U, tipo_transaccion TT 
 where T.id_usuario=U.id_usuario and T.id_transacciones='$_GET[id]' and T.id_tipo_transaccion=TT.id_tipo_transaccion and T.concepto LIKE '%GASTO%'");
...
select P.id_plan_cuentas,
 ...
 and  T.id_transacciones='$_GET[id]'  
 and D.debito>'0.000' 
 order by
 case
...
$usuario = pg_query("select u.nombre_usuario, u.apellido_usuario from usuario u, transacciones t where t.id_usuario=u.id_usuario and t.id_transacciones='$_GET[id]'");
