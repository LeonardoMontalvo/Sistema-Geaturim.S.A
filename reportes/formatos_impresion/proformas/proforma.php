...
$tarifasimpfactura = obtenerTarifasImpuestoProforma($_GET["id"]);
...
if (!empty($tarifasimpfactura)) {
    foreach ($tarifasimpfactura as $key => $value) {
        $pdf->Cell(181, 6, utf8_decode("Tarifa $value[tarifa]"), 0, 0, 'R', 0);
        $pdf->Cell(25, 6, maxCaracter((number_format($value["base_imponible"], 2, ',', '.')), 20), 0, 1, 'C', 0);
    }
} else {
...
if (!empty($tarifasimpfactura)) {
    foreach ($tarifasimpfactura as $key => $value) {
        ...
        $pdf->Cell(181, 6, utf8_decode("IVA $value[tarifa]"), 0, 0, 'R', 0);
        $pdf->Cell(25, 6, maxCaracter((number_format($value["valor_impuesto"], 2, ',', '.')), 20), 0, 1, 'C', 0);
    }
} else {
    $pdf->SetX(1);
...
function obtenerTarifasImpuestoProforma($id)
{
    $sql = "select
    di.cod_impuesto, 
    di.cod_tarifa, 
