...
//	        $this->Cell(122, 4, 'SUPERMERCADO SUPER FIESTA', 0,0, 'R', 0);                                                             
        $this->Ln(7);
        $this->SetX(13);
...
for ($v = 0; $v < $varresult; $v++) {
    $pdf->AddPage();
    $arrayresult = $array[$v];
    $consulta = pg_query("select * from empresa left join factura_venta on empresa.id_empresa  = factura_venta.id_empresa left join clientes on factura_venta.id_cliente=clientes.id_cliente left join tipo_documento on tipo_documento.id_tdocu=clientes.id_tdocu where factura_venta.id_factura_venta='" . $arrayresult . "' ");
...
//		$pdf->Image('C:\xampp\htdocs\syswebfe\images\logo.png',5,10,60); // Img Empresa 
    $pdf->Image('../../../images/'.$_SESSION["parametros_empresa"]["logo_empresa"], 10, 7, 50);
...
//		$pdf->Text(91, 5, utf8_decode('CLAVE DE ACCESO: ')); // Clave de acceso
    ...
    //$pdf->SetX(50);
    //$pdf->SetY(20);
    $pdf->Text(5, 36, "FARMACIA"); // Razon Social Empresa	
    //$pdf->SetY(56);
    //$pdf->SetX(4);	
    //$pdf->SetY(66);	
    //$pdf->SetX(4);	
...
//		$pdf->Text(5, 45, 'Sucursal: '.$direccionEstablecimiento);// Direccion Establecimiento	
    ...
    $x = 58;
    $y = 1;
    $resultado = pg_query("select P.codigo,P.cod_barras, P.articulo, D.cantidad, D.precio_venta, D.descuento_producto, F.tarifa12 from factura_venta F,detalle_factura_venta D  , productos P where  d.cod_productos =P.cod_productos   and D.id_factura_venta = F.id_factura_venta  AND   F.id_factura_venta = '" . $arrayresult . "'");
...
//			$codigoAuxiliar = utf8_decode($row[1]);
        ...
        $pdf->SetY($x);
        $pdf->SetX(5);
        $pdf->multiCell(40, 3, substr($codigo, 0, 18), 1);
        //$pdf->SetY($x);
        //$pdf->SetX(23);
...
//                        }
        ...
        $resultado = pg_query("SELECT F.tarifa12, F.tarifa0, F.tarifa0, F.iva_venta, F.descuento_venta, F.total_venta FROM factura_venta f WHERE id_factura_venta = '" . $arrayresult . "'");
        while ($row = pg_fetch_row($resultado)) {
            $subtotal = $row[0];
            $tarifa = $row[0];
            $tarifa0 = $row[2];
        ...
        // FORMAS DE PAGO	           	
        $resultado = pg_query("SELECT P.codigo, P.descripcion FROM factura_venta F, forma_pagos P WHERE F.id_forma_pago = P.id_forma_pago AND F.id_factura_venta = '" . $arrayresult . "'");
        while ($row = pg_fetch_row($resultado)) {
            $codigovalor = $row[0];
            $valo0 = $row[1];
        }
