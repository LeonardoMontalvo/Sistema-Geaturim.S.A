...
$pdf->Image('../../../images/'.$_SESSION["parametros_empresa"]["logo_empresa"], 10, 5, 21); // Img Empresa
...
for ($i = 0; $i < $numfilas; $i++) {
    ...
    $pdf->SetX(4);
    $pdf->Text(38, 5+$offsety, $rowempre['nombre_empresa'], 0, 0, 'R', 0);
    $pdf->SetFont('Arial', '', 8);



    $pdf->Text(38, 9+$offsety, utf8_decode('' . "RUC:"), 0, 'C', 0); ////CLIENTE (X,Y)   

    $pdf->Text(47, 9+$offsety, utf8_decode('' . strtoupper($rowempre['ruc_empresa'])), 0, 'C', 0); ////CLIENTE (X,Y)

    $pdf->Text(6, 18+$offsety, utf8_decode('' . "Matr.:"), 0, 'C', 0); ////CLIENTE (X,Y)   
    $pdf->Text(12, 18+$offsety, utf8_decode('' . strtoupper($rowempre['direccion_empresa'])), 0, 'C', 0); ////CLIENTE (X,Y)
