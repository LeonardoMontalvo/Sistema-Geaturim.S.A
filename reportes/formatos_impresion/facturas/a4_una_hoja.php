...
//		$logo = $imagen;
    $pdf->Image('../../images/'.$_SESSION["parametros_empresa"]["logo_empresa"], 20, 9, 40); // Img Empresa
...
//		$pdf->Text(108, 81, utf8_decode('CLAVE DE ACCESO: ')); // Clave de acceso
    ...
    $pdf->SetY(53);
    $pdf->SetX(4);
    $pdf->Text(5, 50, utf8_decode('SERVIAGRO ' ));
    ...
    $pdf->Text(5, 83, utf8_decode('Obligado a llevar Contabilidad: ' . $obligado)); // Obligado a llevar contabilidad
  ...
  //  $pdf->Text(5, 89, utf8_decode('Agente de Retención Mediante Resolución Nro. NAC-DNCRASC20-00000001')); //fecha de emision cliente
    ...
    //$pdf->Text(5, 85, utf8_decode('Obligado a llevar Contabilidad: ' . $obligado)); // Obligado a llevar contabilidad
	$pdf->Text(5, 88, utf8_decode('Contribuyente Rimpe Emprendedor con Calificación Artesanal Nro. 159583 ' ));
    $pdf->Rect(3, 101, 205, 20, 'D'); // INFO TRIBUTARIA			     
    $pdf->SetY(105);
    $pdf->SetX(3);
    
	$pdf->multiCell(132, 6, utf8_decode('Razón Social / Nombres y Apellidos: ' . $contribuyente), 0); // Nombre cliente	
   // $pdf->Text(5, 105, utf8_decode('Dirección: ' . $direccion_cliente)); //DIRECCION CLIENTE
	$pdf->Text(135, 108, utf8_decode('RUC / CI:   ' . $identificacion)); // Ruc cliente
...
//			$pdf->multiCell(20, $tam, $codigoAuxiliar,1);
    ...
    if ($pdf->getY() <= 500) {
        ...
        $pdf->SetY($y + 20);
        $pdf->SetX($x);
        $pdf->multiCell(100, 5, utf8_decode("Email: juancarlosriva@hotmail.es " . $email), 0);
