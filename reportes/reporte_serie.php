...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(210, 5, utf8_decode("CONSULTA DE LA SERIE " . $_GET['id'] . "/ PRODUCTO " . $_GET['idp']), 0, 1, 'C', 0);
        $this->SetFont('Amble-Regular', '', 10);
        $this->Ln(8);
...
if ($_GET['id'] != 0) {
    ...
    $sql = pg_query(" select productos.precio_compra,identificacion_pro,empresa_pro,fecha_actual,num_serie from factura_compra,detalle_factura_compra,productos,proveedores where factura_compra.id_proveedor=proveedores.id_proveedor and detalle_factura_compra.cod_productos=productos.cod_productos and factura_compra.id_factura_compra=detalle_factura_compra.id_factura_compra and detalle_factura_compra.cod_productos='$_GET[idp]'");
    while ($row = pg_fetch_row($sql)) {
        $precio_compra = $row[0];
        $ci_proveedor = $row[1];
        $proveedor = $row[2];
        $fecha_compra = $row[3];
        $num_fac_compra = $row[4];
    }
}
...
$pdf->Cell(20, 6, utf8_decode('Factura'), 1, 0, 'C', 1);
$pdf->Cell(20, 6, utf8_decode('F. Venta'), 1, 0, 'C', 1);
$pdf->Cell(16, 6, utf8_decode('P. Venta'), 1, 0, 'C', 1);
$pdf->Cell(54, 6, utf8_decode('Producto'), 1, 0, 'C', 1);
$pdf->Cell(20, 6, utf8_decode('N. Compra'), 1, 0, 'C', 1);
$pdf->Cell(20, 6, utf8_decode('F. Compra'), 1, 0, 'C', 1);
$pdf->Cell(40, 6, utf8_decode('Proveedor'), 1, 0, 'C', 1);
$pdf->Cell(20, 6, utf8_decode('P. Compra'), 1, 1, 'C', 1);
$pdf->SetFont('helvetica', '', 8.5);
$pdf->Cell(20, 6, ($num_fac_venta), 0, 0, 'C', 0);
$pdf->Cell(20, 6, ($fecha_venta), 0, 0, 'C', 0);
$pdf->Cell(16, 6, number_format($precio_venta, 2, ',', '.'), 0, 0, 'R', 0);
$pdf->Cell(54, 6, maxCaracter($descripcion, 50), 0, 0, 'L', 0);
$pdf->Cell(20, 6, substr($num_fac_compra, 8, 30), 0, 0, 'C', 0);
$pdf->Cell(20, 6, $fecha_compra, 0, 0, 'C', 0);
$pdf->Cell(40, 6, maxCaracter($proveedor, 20), 0, 0, 'L', 0);
$pdf->Cell(20, 6, number_format($precio_compra, 2, ',', '.'), 0, 0, 'R', 0);
