...
if ($_GET['id_empre'] != '0') {
    ...
    where estado='Activo' $query_punto
   ...
   "
        ...
        $saldo_anterior = pg_fetch_row(pg_query(
                ...
                INNER JOIN detalle_transaccion D USING(id_transacciones) 
                INNER JOIN plan_cuentas P USING(id_plan_cuentas)
                WHERE D.id_plan_cuentas = '$_GET[id_plan]' AND T.fecha_registro::date between '$_GET[inicio]' and '$_GET[fin]' AND T.estado='Activo'"
        ));
...
$query_detalle = pg_query(
        "        SELECT t.id_transacciones,T.fecha_registro,t.comprobante,identificador_cli_pro, p_g.empresa_pro as empresa_g,c.nombres_cli,t.concepto as concepto_trans,
 ...
 p_c.empresa_pro as empresa_comp, string_agg(fpc.numero_documento,',')
            ...
            left JOIN proveedores p_c   on p_c.id_proveedor=fc.id_proveedor
             ...
             left JOIN clientes c on c.id_cliente=fv.id_cliente
              ...
              dt.id_plan_cuentas = '$_GET[id_plan]'  AND T.fecha_registro between  '$_GET[inicio]' and '$_GET[fin]'
            AND T.estado='Activo'
...
//////////////////////////
    ...
    if ($row1[3] == 'VEN') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4 . "---" . $row1[14]), 90), 0, 0, 'L', 0); //CONCE
    }
    if ($row1[3] == 'COM') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4 . "---" . $row1[13]), 105), 0, 0, 'L', 0); //CONCE
    }
    if ($row1[3] == 'GAS') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4 . "---" . $row1[12]), 105), 0, 0, 'L', 0); //CONCE
    }
    if ($row1[3] == 'CxP') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4), 105), 0, 0, 'L', 0); //CONCE
    }
    if ($row1[3] == 'CxC') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4), 105), 0, 0, 'L', 0); //CONCE
    }
  if ($row1[3] == 'OTRO') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4), 105), 0, 0, 'L', 0); //BENEFICIA
    }
 if ($row1[3] == 'ING') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4), 90), 0, 0, 'L', 0); //BENEFICIA
    }
 if ($row1[3] == 'EGR') {
        $pdf->Cell(150, 6, maxCaracter(utf8_decode($pos4), 90), 0, 0, 'L', 0); //BENEFICIA
    }
 if ($row1[3] == 'ANTP') {
    ...
    $pdf->Cell(13, 6, ($row1[7]), 0, 0, 'R', 0); //HABER
    $pdf->Cell(13, 6, ($row1[8]), 0, 0, 'R', 0); //HABER
    $sub_haber_conci += $row1[8];
    $sub_debe_conci += $row1[7];
