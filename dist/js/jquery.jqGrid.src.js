...
$.jgrid.extend({
	...
	clearBeforeUnload : function () {
		return this.each(function(){
			...
			//$(document).off("mouseup"); // TODO add namespace
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				// sorting stuff
				$($t).on("jqGridOnSortCol.setFrozenColumns", function (e, index, idxcol) {
				...
				if($t.p.hoverrows === true) {
					$("#"+$.jgrid.jqID($t.p.id)).off("mouseover").off("mouseout");
				}
...
var xmlJsonClass = {
	...
	toJson: function(o, name, ind, card card-bodyform) {
		if(card card-bodyform === undefined) card card-bodyform = true;
		var json = name ? ("\"" + name + "\"") : "", tab = "\t", newline = "\n";
		if(!card card-bodyform) {
			tab= ""; newline= "";
		}
		...
		if (o === "[]") {
			...
			for (i = 0, n = o.length; i < n; i += 1) {
				ar[i] = this.toJson(o[i], "", ind + tab, card card-bodyform);
			}
			...
			for (m in o) {
				if (o.hasOwnProperty(m)) {
					arr[arr.length] = this.toJson(o[m], m, ind + tab, card card-bodyform);
			}
		}
