...
$Archivo = "detalle_facturas_ventas.xls";
...
$objPHPExcel->getProperties()->setCreator("P&S Systems")
        ...
        ->setSubject("RESUMEN DE FACTURAS VENTAS GENERAL")
        ->setDescription("")
        ->setKeywords("")
        ->setCategory("");
...
$y = 7;
    ...
    $consulta1 = pg_query("select num_factura,factura_venta.fecha_actual,hora_actual,fecha_cancelacion,tipo_precio,forma_pago,tarifa0,tarifa12,iva_venta,descuento_venta,total_venta,identificacion,nombres_cli,nombre_empresa,id_factura_venta, factura_venta.estado from factura_venta, clientes,empresa,usuario where factura_venta.id_cliente=clientes.id_cliente and factura_venta.id_empresa='$_GET[id]'  and usuario.id_usuario=factura_venta.id_usuario and factura_venta.fecha_actual between '$_GET[inicio]' and '$_GET[fin]' and factura_venta.estado='Activo' order by factura_venta.id_factura_venta asc");
        ...
        while ($row1 = pg_fetch_row($consulta1)) {
            if ($repetido == 0) {
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue("O" . $y, 'Estado');
                $objPHPExcel->getActiveSheet()->getStyle("B" . $y . ":O" . $y)->getFont()->setBold(true)->setName('Verdana')->setSize(10);
                $objPHPExcel->getActiveSheet()->getStyle("B" . $y . ":O" . $y)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $repetido = 1;
                $styleArray = array(
                    'borders' => array(
                        'bottom' => array(
                ...
                $objPHPExcel->getActiveSheet()->getStyle('B' . $y . ':O' . $y)->applyFromArray($styleArray);
                unset($styleArray);
                $y++;
            }
...
$objPHPExcel->getActiveSheet()->getStyle('B' . ($y - 1) . ':O' . ($y - 1))->applyFromArray($styleArray);
unset($styleArray);
//$y=$y+1;  
$objPHPExcel->setActiveSheetIndex(0)
