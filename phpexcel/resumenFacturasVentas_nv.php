...
$Archivo = "reporte_facturas_ventas_clientes.xls";
...
$objPHPExcel->getProperties()->setCreator("P&S Systems")
        ...
        ->setSubject("RESUMEN DE NOTAS DE VENTA POR CLIENTES")
        ->setDescription("")
        ->setKeywords("")
        ->setCategory("");
...
$objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue("B2", 'RESUMEN DE NOTAS DE VENTA POR CLIENTES');
...
while ($row = pg_fetch_row($consulta)) {
    $consulta1 = pg_query("select comprobante,facturas_novalidas.fecha_actual,hora_actual,facturas_novalidas.fecha_actual,tipo_precio,forma_pago,tarifa0,tarifa12,iva_venta,descuento_venta,total_venta,
identificacion,nombres_cli,nombre_empresa,id_facturas_novalidas from facturas_novalidas, clientes,empresa,usuario where facturas_novalidas.id_cliente=clientes.id_cliente 
and facturas_novalidas.id_empresa='$_GET[id]' and usuario.id_usuario=facturas_novalidas.id_usuario and facturas_novalidas.id_cliente='$row[0]' and facturas_novalidas.fecha_actual
 between '$_GET[inicio]' and '$_GET[fin]' and facturas_novalidas.estado='Activo'  order by facturas_novalidas.id_facturas_novalidas asc
");
        ...
        while ($row1 = pg_fetch_row($consulta1)) {
            if ($repetido == 0) {
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValueExplicit("C" . $y, $row1[11])
                        ->setCellValueExplicit("F" . $y, $row1[12], PHPExcel_Cell_DataType::TYPE_STRING);
                $y++;
                $objPHPExcel->setActiveSheetIndex(0)
                        ->setCellValue("B" . $y, 'Sección: ')
                        ->setCellValue("C" . $y, $row1[13]);
                $y++;
                $y++;
                $objPHPExcel->setActiveSheetIndex(0)
