...
$objPHPExcel->getProperties()->setCreator("P&S Systems")
        ...
        ->setSubject("RESUMEN DE FACTURAS VENTAS POR CLIENTES")
        ->setDescription("")
        ->setKeywords("")
        ->setCategory("");
...
$objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue("B2", 'RESUMEN DE FACTURAS VENTAS POR CLIENTES');
...
while ($row = pg_fetch_row($consulta)) {
    $consulta1 = pg_query("select num_factura,factura_venta.fecha_actual,hora_actual,fecha_cancelacion,tipo_precio,forma_pago,tarifa0,tarifa12,iva_venta,descuento_venta,total_venta,identificacion,nombres_cli,nombre_empresa,id_factura_venta from factura_venta, clientes,empresa,usuario where factura_venta.id_cliente=clientes.id_cliente and factura_venta.id_empresa='$_GET[id]' and usuario.id_usuario=factura_venta.id_usuario and factura_venta.id_cliente='$row[0]' and factura_venta.fecha_actual between '$_GET[inicio]' and '$_GET[fin]' and factura_venta.estado='Activo'  order by factura_venta.id_factura_venta asc");
