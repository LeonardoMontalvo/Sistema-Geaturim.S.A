...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue("B2", "RESUMEN GENERAL CUENTAS POR COBRAR ($tipoCuenta)");
        ...
        if ($_GET['tipo'] == 'Externas') {
            ...
            if (pg_num_rows($sql)) {
                while ($row1 = pg_fetch_assoc($sql)) {
                    $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue("H" . $y, utf8_decode($row1["tipo_documento"]))
            ...
            $sql = pg_query(
                "(
                    ...
                    $id_usuario_fv    $query_punto order by fecha_actual asc
                )
                union all
                (
                    SELECT 
            ...
            if (pg_num_rows($sql)) {
                while ($row1 = pg_fetch_assoc($sql)) {
                    $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue("G" . $y, utf8_decode($row1["vence"]))
                        ->setCellValue("H" . $y, utf8_decode($row1["tipo_documento"]))
                        ->setCellValue("I" . $y, round($row1["monto_credito"], 2, PHP_ROUND_HALF_EVEN))
                        ->setCellValue("J" . $y, round($row1["abonos"], 2, PHP_ROUND_HALF_EVEN))
                        ->setCellValue("K" . $y, round($row1["saldo"], 2, PHP_ROUND_HALF_EVEN));
                    $objPHPExcel->getActiveSheet()->getStyle("B" . $y . ":K" . $y)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    $y = $y + 1;
...
function obtenerCuentasInternasExternas($idcliente)
{
    ...
    (
        SELECT num_factura,
            ...
            $id_usuario_fv    $query_punto and pv.tipo_documento='Factura' order by fecha_actual asc
    )
    union all
    (
        ...
        FROM facturas_novalidas fv
            ...
            inner join pagos_venta pv on id_factura_venta=fv.id_facturas_novalidas
            where c.id_cliente=$idcliente AND fv.fecha_actual $query_fecha '$_GET[fin]' 
            $id_usuario_fv    $query_punto and pv.tipo_documento='Nota' order by fecha_actual asc
    )
    ";
