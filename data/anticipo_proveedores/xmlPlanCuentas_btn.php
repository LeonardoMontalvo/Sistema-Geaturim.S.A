...
} elseif ($_GET['id'] == "Cheque" || $_GET['id'] == "Transferencias") {
...
} elseif ($_GET['id'] == "anticipo_clientes" ) {
    
      if ($search == 'false') {
        $SQL = "select * from plan_cuentas where cuenta='M'  and  (descripcion LIKE '%ANTICIPO A PROVEEDORES%')  ORDER BY $sidx $sord offset $start limit $limit";
    } else {
        if ($_GET['searchOper'] == 'eq') {
            $SQL = "select * from plan_cuentas where $_GET[searchField] = '$_GET[searchString]'  ORDER BY $sidx $sord offset $start limit $limit";
        }
