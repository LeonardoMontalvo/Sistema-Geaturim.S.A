...
include_once __DIR__ . "/../../procesos/fun_obtener_parametros_pv.php";
error_reporting(0);
$page = $_GET['page'];
$limit = $_GET['rows'];
$sidx = $_GET['sidx'];
...
$sqlcount = "SELECT COUNT(*) AS count FROM productos p INNER JOIN unidad_medida_productos ump on P.cod_productos=ump.cod_productos";

$SQL = " 
SELECT   DISTINCT ON (p.cod_productos)  P.cod_productos, 
P.codigo,
P.cod_barras,
P.articulo,
P.iva,
P.series,
P.precio_compra,
P.utilidad_minorista,
P.iva_minorista,
P.utilidad_mayorista,
P.iva_mayorista, 
g.nombre_generico,
c.nombre_categoria,
P.descuento,
...
INNER JOIN unidad_medida_productos ump on P.cod_productos=ump.cod_productos";
...
if ($search != 'false') {
    ...
    if ($_GET['searchOper'] == 'eq') {
        $SQL .= " WHERE $campo = '$_GET[searchString]'";
        $sqlcount .= " WHERE $campo = '$_GET[searchString]'";
    }
    if ($_GET['searchOper'] == 'ne') {
        $SQL .= " WHERE  where $campo != '$_GET[searchString]'";
        $sqlcount .= " WHERE  where $campo != '$_GET[searchString]'";
    }
    if ($_GET['searchOper'] == 'bw') {
        $SQL .= " WHERE $campo like '$_GET[searchString]%'";
        $sqlcount .= " WHERE $campo like '$_GET[searchString]%'";
    }
    if ($_GET['searchOper'] == 'bn') {
        $SQL .= " WHERE $campo not like '$_GET[searchString]%'";
        $sqlcount .= " WHERE $campo not like '$_GET[searchString]%'";
    }
...
$SQL=consultaProductos($SQL);
$sqlcount=consultaProductos($sqlcount);


//$result = pg_query("SELECT COUNT(*) AS count FROM productos");
$result = pg_query($sqlcount);
...
$SQL .= " ORDER BY p.$sidx $sord offset $start limit $limit";
$result = pg_query($SQL);
...
function consultaProductos($SQL)
{
    $aux = explode("WHERE", $SQL);
    $parametros = obtenerParametrosPv($_SESSION['PV']);
    if ($parametros["mostrar_productos_configurados"] == "t") {
        if (count($aux) > 1) {
            $parte1 = $aux[0];
            $parte2 = $aux[1];
            $parte1 .= "  left join productos_excluidos_pv pep
            on p.cod_productos=pep.cod_productos 
            and pep.id_punto_venta=$_SESSION[PV]";
            $parte2 .= " and pep.cod_productos is null";
            $SQL = $parte1 ." WHERE ". $parte2;
        } else {
            $SQL .= " 
            left join productos_excluidos_pv pep
            on p.cod_productos=pep.cod_productos 
            and pep.id_punto_venta=$_SESSION[PV]
            WHERE pep.cod_productos is null
            ";
        }
    }
    return $SQL;
