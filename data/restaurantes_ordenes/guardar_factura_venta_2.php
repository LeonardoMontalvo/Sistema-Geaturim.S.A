...
if (isset($_POST['enviarxml']) == "enviarxml") {
    ...
    exec('c:\xampp\htdocs\syswebfe\firma\pruebasFE ' . '../../xmls/fac', $resultado);
    $respuesta = consultarComprobante($ambiente, $consult_clave);
    print_r($respuesta);
    if (isset($respuesta->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->estado)) {
        if ($respuesta->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->estado == 'AUTORIZADO') {
...
if (isset($_POST['enviarxmlguia']) == "enviarxmlguia") {
    ...
    //    print_r($result);
    $consulta = generarFirma($result, $consult_clave, 'guiaRemision', $pass, $token, $ambiente);  // devuelvo archivo firmado en formato XADESBES
    //    print_r($consulta);
    if ($consulta == 5) {
        $data = 5; // Error ruta Archivo
    } else {
        if ($consulta == 6) {
            $data = 6; // Contraseña de token Incorrecta
        } else {
            $respWeb = webService($consulta, $ambiente, $consult_clave, '', 'guiaRemision', $pass, $token, '0'); // ENVIO EL ARCHIVO XML PARA VALIDAR   
            //            print_r($respWeb);
            if (isset($respWeb['RespuestaRecepcionComprobante']['estado'])) {
                ...
                pg_query("UPDATE guia_remision SET estado = '8' where id_guia_remision = '" . $_POST['id'] . "'"); // ERROR EN EL WEB SERVICE
            }
        }
    }
    $item = array('estado' => $data, 'id' => $_POST['id']);
    ...
    if ($_POST["tipo_venta"] == "FACTURA") {
        ...
        if ($_POST['id_cliente'] == "") {
            ...
            if (strlen($tipo) == 10) {
                ...
                if ($forma == 'otros') {
                    //	 echo '<br>GUARDAR FACTURA VENTA33: <br>' . "insert into factura_venta values('$cont1','$conpuntoresult','$contt','$_SESSION[id]','$cont1','$_POST[num_factura]',"
                ...
                if (strlen($tipo) == 13) {
                    // guardar clientes
                    /* pg_query("insert into clientes values('$contt','Ruc','$_POST[ruc_ci]','$_POST[nombre_cliente]','natural',"
...
and (formas_pago_mixto.forma_pago='CREDITO' or formas_pago_mixto.forma_pago='CPOSFECHADO') GROUP BY formas_pago_mixto.forma_pago
                ...
                if ($_POST['id_cliente'] == "") {
                    ...
                    //                    echo '<br>GUARDAR FACTURA pagos_venta: <br>' . "insert into pagos_venta values('$cont2','$idCli','$cont1','$_SESSION[id]','$_POST[fecha_actual]','$adelanto','$meses',"
...
and (formas_pago_mixto.forma_pago='CHEQUE'  or formas_pago_mixto.forma_pago='CONTADO') GROUP BY formas_pago_mixto.forma_pago
                ...
                $consulta_mixto = pg_query("select formas_pago_mixto.forma_pago,formas_pago_mixto.valor from factura_venta, formas_pago_mixto where factura_venta.id_factura_venta=formas_pago_mixto.id_factura_venta and factura_venta.id_factura_venta='$cont1' and formas_pago_mixto.forma_pago='CREDITO'");
                while ($row = pg_fetch_row($consulta_mixto)) {
                ...
                $consulta_mixto = pg_query("select formas_pago_mixto.forma_pago,formas_pago_mixto.valor from factura_venta, formas_pago_mixto where factura_venta.id_factura_venta=formas_pago_mixto.id_factura_venta and factura_venta.id_factura_venta='$cont1' and formas_pago_mixto.forma_pago='CPOSFECHADO'");
                while ($row = pg_fetch_row($consulta_mixto)) {
  ...
  and detalle_factura_venta.id_factura_venta='$cont1' and detalle_factura_venta.bien_servicio='B'");
        ...
        if ($_POST["tipo_venta"] == "NOTA") {
            ...
            if ($_POST['id_cliente'] == "") {
                ...
                if (strlen($tipo) == 10) {
                    ...
                    // guardar clientes  
                    pg_query("insert into clientes values('$contt','Cedula','$_POST[ruc_ci]','$_POST[nombre_cliente]','natural','$_POST[direccion_cliente]',"
                        . "'$_POST[telefono_cliente]','','','','$_POST[correo]','1','','Activo')");
                    // fin 
                    ...
                    if (strlen($tipo) == 13) {
                        // guardar clientes   
                        pg_query("insert into clientes values('$contt','Ruc','$_POST[ruc_ci]','$_POST[nombre_cliente]','natural','$_POST[direccion_cliente]','$_POST[telefono_cliente]','','','','$_POST[correo]','1','','Activo')");
                        // fin
                        $conpunto = 1;
                        $consultapunto = pg_query("select max(id_punto_venta_empresa) from punto_venta_empresa where punto_venta_empresa.id_usuario='$_SESSION[id]'");
                        while ($row = pg_fetch_row($consultapunto)) {
    ...
    $consulta_empresa = pg_query("select ruc_empresa,clave, token,direccion_empresa from empresa where id_empresa = 1");
    while ($row = pg_fetch_row($consulta_empresa)) {
        $ruc = $row[0];
        $direc_guia = $row[3];
        $pass = $row[1];
    ...
    pg_query("insert into guia_remision values('$contguias','$_POST[id_fac]','$_POST[transportistaguia]','$_POST[fecha_actual]','$_POST[fecha_actual]','$_POST[fecha_actual]','$_POST[hora_actual]','$_POST[num_guia]','','$clave','','$direc_guia','$_POST[direccion_cliente]','VENTA','$_POST[hora_actual]','$_POST[hora_actual]','$direc_guia.$_POST[direccion_cliente]','','','$conpuntoresult','$_POST[num_guia_remision]')");
    $result = generarXMLGUIA($contguias, $codDoc, $ambiente, $emision);
    //    print_r($result);
    $consulta = generarFirma($result, $clave, 'guiaRemision', $pass, $token, $ambiente);  // devuelvo archivo firmado en formato XADESBES
    //    print_r($consulta);
    if ($consulta == 5) {
        $data = 5; // Error ruta Archivo
    } else {
        if ($consulta == 6) {
            ...
            $respWeb = webService($consulta, $ambiente, $clave, '', 'guiaRemision', $pass, $token, '0'); // ENVIO EL ARCHIVO XML PARA VALIDAR   
            //            print_r($respWeb);
            if (isset($respWeb['RespuestaRecepcionComprobante']['estado'])) {
                $respuesta = consultarComprobante($ambiente, $clave);
                if (isset($respuesta->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->estado)) {
                    ...
                    //                    $data = $respuesta['RespuestaAutorizacionComprobante']['autorizaciones']['autorizacion']['estado'];
                    pg_query("UPDATE guia_remision SET estado = '7' where id_guia_remision = '$contguias'"); // Rechazado
                }
            } else {
                $data = $respWeb['RespuestaRecepcionComprobante']['comprobantes']['comprobante']['mensajes']['mensaje']['mensaje'];
                pg_query("UPDATE guia_remision SET estado = '8' where id_guia_remision = '$contguias'"); // ERROR EN EL WEB SERVICE
            }
        }
    }
