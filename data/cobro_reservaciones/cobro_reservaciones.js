...
function cargar_facturas() {
    ...
    if (id === "") {
        ...
        $("#list2").jqGrid('setGridParam', {
            url: 'xmlReservaciones.php?id_cliente=' + id,
            datatype: 'xml'
        }).trigger('reloadGrid');
        $("#buscar_reservaciones").dialog("open");
    }
}
...
function guardar_pagos() {
    ...
    if ($("#id_cliente").val() === "") {
        ...
        if ($("#ruc_ci").val() === "") {
            ...
            if ($("#valor_pagado").val() === "" || $("#valor_pagado").val() <= 0) {
                $("#valor_pagado").focus();
                alertify.error("Error... Ingrese el valor de pago");
            } else {
                var saldo_pago = $("#saldo2").val() - $("#valor_pagado").val();
                $.ajax({
                    type: "POST",
                    url: "guardar_pago_reservacion.php",
                    data: "ids=" + $("#ids").val() + "&fecha_actual=" + $("#fecha_actual").val() + "&hora_actual=" + $("#hora_actual").val() + "&valor=" + $("#valor_pagado").val() + "&saldo=" + saldo_pago,
                    success: function (data) {
                        ...
                        if (val > 0) {
                            alertify.alert("Pago Guardado correctamente", function () {
                                ...
                                $("#valor_pagado").val("");
                                $("#valor_pagado").focus();
                                $("#saldo2").val(saldo_pago);
                                window.open("../../reportes/reportePagoReservacion.php?id=" + $("#ids").val(), '_blank');
                            });
                        }
                    }
                });
...
function inicio() {
    ...
    $("#btnImprimir").click(function () {
        ...
        if ($("#ids").val() != "") {
            window.open("../../reportes/reporteReservacionGeneral.php?id=" + $("#ids").val(), '_blank');
        } else {
    ...
    $("#buscar_reservaciones").dialog(dialogo);
    ...
    $("#forma_pago").on("change", function () {
        if ($("#forma_pago").val() == "CHEQUE" || $("#forma_pago").val() == "TARJETA") {
    ...
    jQuery("#list2").jqGrid({
        ...
        shrinkToFit: true,
        sortorder: 'asc',
        caption: 'Lista de Reservaciones',
        viewrecords: true,
        ondblClickRow: function (rowid) {
            ...
            if (id) {
                ...
                //////////////////////
                $("#buscar_reservaciones").dialog("close");
                $("#tablaNuevo tbody").empty();
                $.ajax({
                    type: "POST",
                    url: "buscar_pagos.php",
