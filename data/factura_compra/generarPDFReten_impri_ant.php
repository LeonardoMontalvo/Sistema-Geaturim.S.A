...
function generarPDFReten($id) {
    conectarse();
    $consulta = pg_query("select * from empresa left join factura_compra on empresa.id_empresa  = factura_compra.id_empresa left join retencion_fuente_factura_compra on retencion_fuente_factura_compra.id_factura=factura_compra.id_factura_compra left join proveedores on factura_compra.id_proveedor=proveedores.id_proveedor left join tipo_documento on tipo_documento.id_tdocu=proveedores.id_tdocu where retencion_fuente_factura_compra.id_factura = '" . $id . "' and  retencion_fuente_factura_compra.id_gastos='1'");

    while ($row = pg_fetch_row($consulta)) {
        $ruc = $row[2];
        $fecha_registro_retencion = $row[29];
           $numeroAutorizacion = $row[62];

        if ($numeroAutorizacion == "") {
            $numeroAutorizacion = $row[60];
        } else {
            $numeroAutorizacion = $row[62];
        }
    ...
    $pdf->Image('../../images/'.$_SESSION["parametros_empresa"]["logo_empresa"], 1, 5, 65); // Img Empresa 
    ...
    $pdf->Ln(2);
    $pdf->SetX(3);
    $pdf->Rect($pdf->GetX(), $pdf->GetY(), 70, 18, 'D'); ////3 INFO ADICIONAL
    $y = $pdf->GetY();
    $x = $pdf->GetX();
