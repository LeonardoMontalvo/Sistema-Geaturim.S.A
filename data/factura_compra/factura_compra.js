...
function guardar_factura() {
    if ($("#proveedor_con_retencion").val() == 'Si') {
...
function flecha_atras() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var val = data;
            if (val != "") {
                $("#ivax").attr("disabled", true);$("#ivax").attr("disabled", true);
...
function eliminar_retencion(e) {
    if ($("#id_comprobante_serie").val() == "") {
        ...
        $.ajax({
            ...
            data: "fecha_emision_factura=" + $("#fecha_emision").val(),
            success: function (data) {
                var val = data;
                console.log(val + "ggg");
                if (val == "22") {
...
function inicio() {
        ...
        if (tipo == "Cedula") {
            ...
            $("#ruc_ci").autocomplete({
                ...
                focus: function (event, ui) {
                    ...
                    $("#proveedor_con_retencion").val(ui.item.principal);
                    return false;
                },
    ...
    let ivaxval = $("ivax").val();
    document.getElementById("ivax").addEventListener("input", function (e) {
        let totx = $("#subx").val();
        let val = $(this).val();

        var totivax = Number(val) + Number(totx);
        $("#iva").val(val);
        ///////////TOTALES////////////////
        $("#tot").val(totivax);
        $("#totx").val(totivax);
...
function calcularIva(valor, tarifa = null) {
    if (!tarifa) {
        tarifa = calculoIVA;
    }
    let result = (valor * (Number(tarifa) / 100)).toFixed(4);
