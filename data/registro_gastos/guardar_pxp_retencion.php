...
{
    $sql = "SELECT  id_pagos_compra,monto_credito,saldo FROM pagos_compra where  (estado='Activo' or estado='Cancelado') and id_factura_compra='$idfactura' and tipo_documento='FACTURA' and comprao_gasto='G'";
    $res = pg_query($sql);
    $rows = pg_fetch_all($res);
    if (empty($rows)) {
        return [];
...
{
    $sql = "select id_proveedor,fecha_actual, num_serie from gastos where id_gastos=$idfactura";
    $res = pg_query($sql);
    $rows = pg_fetch_all($res);
    if (empty($rows)) {
        return [];
...
{
    $sql = "
    select fv.id_gastos from gastos fv
    inner join formas_pago_mixto_g fp
    on fv.id_gastos=fp.id_gastos
    where fp.forma_pago='CREDITO'
    and fv.id_gastos=$idfactura
    ";
    $res = pg_query($sql);
...
function guardarPagoP($idfactura, $formap, $tipop, $valorp, $obs, $banco, $fecha)
{
    ...
    VALUES ($id, $idproveedor, $usuario, $comprobante, '$fecha', 
            ...
            'Activo', $idfactura, $conpuntoresult, 'G');
    ";
    var_dump($sql);
    $res = pg_query($sql);
    if (empty($res)) {
