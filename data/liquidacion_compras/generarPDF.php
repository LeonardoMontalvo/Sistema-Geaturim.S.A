...
//	        $this->Cell(145, 5, 'CUERPO DE BOMBEROS DE COTACACHI', 0,0, 'R', 0);                                                             
	        $this->Ln(7);
	        $this->SetX(13);
	...
	function generarPDFcorreo($id) { 
		...
		while ($row = pg_fetch_row($consulta)) {
			...
			$obligado = $row[17];
			$contribuyente = $row[63];
			$identificacion = $row[62];
			$direcion = $row[65];
			$telefono = $row[66];
			$email = $row[70];			
			$secuencial = $row[29];
...
//		$pdf->Image('D:\xampp\htdocs\syswebfeb\images\logo.png',30,5,40); // Img Empresa 
		...
		$pdf->Text(108, 21, utf8_decode("LIQUIDACIÓN DE COMPRA DE BIENES"),1,1, 'L',true);
                $pdf->Text(108, 25, utf8_decode("Y PRESTACIÓN DE SERVICIOS"),1,1, 'L',true);// Tipo comprobante
...
//	
		...
		$pdf->Text(9, 117, utf8_decode('Fecha de Emisión: '.$fechaEmision)); //fecha de emision cliente
...
$pdf->SetFillColor(170,170,170); 
		...
		$pdf->SetY(123);
		$pdf->SetX(58);
		$pdf->multiCell( 78, 5, utf8_decode('Descripción'),1,1, 'L',true);
		$pdf->SetY(123);
		$pdf->SetX(136);
		$pdf->multiCell( 17, 5, utf8_decode('Precio U.'),1,1, 'L',true);
		$pdf->SetY(123);
		$pdf->SetX(153);
		$pdf->multiCell( 18, 5, utf8_decode('Descu.%'),1,1, 'L',true);
		$pdf->SetY(123);
		$pdf->SetX(171);
		$pdf->multiCell( 20, 5, utf8_decode('Total'),1,1, 'L',true);
...
//			$pdf->multiCell(20, $tam, $codigoAuxiliar,1);
		...
		if($pdf->getY() <= 500) {
			$pdf->Ln(5);
			$pdf->SetX(8);		   
		    $pdf->Rect($pdf->GetX(), $pdf->GetY(), 100, 25 , 'D');////3 INFO ADICIONAL	   
			$y =  $pdf->GetY();
			$x =  $pdf->GetX();	
			$y1 =  $pdf->GetY();
			$x1 =  $pdf->GetX();	
...
//                        }
			...
			$pdf->SetY($y1 + 6);
			$pdf->SetX($x1);
			$pdf->multiCell(45, 6, utf8_decode("Subtotal IVA 0 %"),1,1, 'L',true);	
			$pdf->SetY($y1 + 6);
			$pdf->SetX($x1 + 45);
			...
			$pdf->SetY($y1 + 24);
			$pdf->SetX($x1);
			$pdf->multiCell(45, 6, utf8_decode("PROPINA"),1,1, 'L',true);	
			$pdf->SetY($y1 + 24);
			$pdf->SetX($x1 + 45);
			$pdf->multiCell(33, 6, utf8_decode("0.00"),1);
			$pdf->SetY($y1 + 30);
			$pdf->SetX($x1);
			$pdf->multiCell(45, 6, utf8_decode("TOTAL"),1,1, 'L',true);
			$pdf->SetY($y1 + 30);
			$pdf->SetX($x1 + 45);
