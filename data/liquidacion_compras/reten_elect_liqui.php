...
function generarXMLRETLIQUI($id, $codDoc, $ambiente, $emision) {

     $consulta = pg_query("SELECT nombre_empresa, ruc_empresa, direccion_empresa, obligacion, establecimiento,
        punto_emision, g.id_liquidacion_compra, comprobante, fecha_actual, fecha as fecha_aut, nombre_comercial,
        g.num_factura as sec_doc, rffc.num_serie, rffc.clave, rffc.num_autorizacion, 
        identificacion_pro, correo, empresa_pro, direccion_pro, codigo_tdocu, 
        case when telefono!='' then telefono else celular end as telefono_pro
        from empresa e inner join liquidacion_compra g using(id_empresa)
        inner join retencion_fuente_factura_compra rffc on rffc.id_factura=g.id_liquidacion_compra 
   ...
   while ($row = pg_fetch_assoc($consulta)) {
        ...
        $id_fact = $row['id_liquidacion_compra'];
        ...
        $fechaEmision_act = $row['fecha_actual'];
        $date = new DateTime($fechaEmision_act);
        // $fechaEmisionfinal_act = $date->format('d/m/Y');
        // $fechaAut = $row[31];
    ...
    $consultaretencion = pg_query("select 
       ...
       inner join retencion_fuentes TR on CD.id_retencion_fuentes = TR.id_retencion_fuentes where CR.id_factura= $id_fact and CR.id_gastos='20' ");
    //echo $sql;
    $totalSinImpuestos = 0;
    $valporcentiva = 0;
...
function generarXMLCDATAFACLIQUI($data) {
    $s = "";
    $s = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
    $s .= "<autorizacion>\n";
