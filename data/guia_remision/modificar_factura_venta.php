...
error_reporting(0);
    ...
    if ($forma == "Credito") {
        ...
        // fin
        pg_query("update factura_venta Set forma_pago='Credito' where id_factura_venta='".$cont1."'"); 
        // guardar meses
        if ($meses > 1) {
            for ($i = 1; $i <= $meses - 1; $i++) {
        ...
        pg_query("update factura_venta Set forma_pago='$forma' where id_factura_venta='".$cont1."'");
    }
    ...
    while ($row = pg_fetch_row($consulta)) {
        ...
        pg_query("insert into kardex_valorizado values(".$cont_v.",'".$id."','$_POST[fecha_actual]', '" . 'Mod Venta:'.$_POST['num_serie'].'-'. $_POST['num_factura']."','".$cantidad_salida."','','".$cantidad."','".$precio_unitario_salida."','".$precio_total_salida."','','','".$cantidad_total."','4')");
        // fin
                   
        // guardar kardex
        pg_query("insert into kardex values('$cont_k','$_POST[fecha_actual]', '" . 'Mod F.V:'.$_POST['num_serie'].'-'. $_POST['num_factura']."' ,'$canti1','$precio_unitario_salida','$precio_total_salida','$id','$cal1','2','','')");
        // fin
    }
    // fin suma
    ...
    for ($i = 0; $i <= $nelem; $i++) {
        ...
        pg_query("insert into kardex_valorizado values(".$cont_v.",'".$arreglo1[$i]."','$_POST[fecha_actual]', '" . 'N Venta:'.$_POST['num_serie'].'-'. $_POST['num_factura']."','','".$cantidad_salida."','".$cantidad."','".$precio_unitario_salida."','".$precio_total_salida."','','','".$cantidad_total."','4')");
        // fin

        // guardar kardex
        pg_query("insert into kardex values('$cont_k','$_POST[fecha_actual]', '" . 'N F.V:'.$_POST['num_serie'].'-'. $_POST['num_factura']."' ,'$arreglo2[$i]','$arreglo3[$i]','$arreglo5[$i]','$arreglo1[$i]','$cal','2','','')");
        // 
    }
