...
if ($_GET['id'] == "Contado_reten") {
...
} elseif ($_GET['id'] == "Cheque_reten" || $_GET['id'] == "Transferencias_reten") {
    ...
    if ($search == 'false') {
      ...
      if ($search == 'false') {
        $SQL = "select * from plan_cuentas where cuenta='M'  and  ( descripcion like '%CLI%') and cuenta='M' and codigo_plan like '%1.1.02.05.01%'   ORDER BY $sidx $sord offset $start limit $limit";
    } else {
        if ($_GET['searchOper'] == 'eq') {
            $SQL = "select * from plan_cuentas where $_GET[searchField] = '$_GET[searchString]'  ORDER BY $sidx $sord offset $start limit $limit";
        }
...
elseif ($_GET['id'] == "cxp" ) {
      if ($search == 'false') {
        $SQL = "select * from plan_cuentas where cuenta='M'  and  (descripcion like '%FTE. POR PAGAR%'  or descripcion like '%PROVEE%') and codigo_plan  like '%2.1%' ORDER BY $sidx $sord offset $start limit $limit";
    } else {
        if ($_GET['searchOper'] == 'eq') {
            $SQL = "select * from plan_cuentas where $_GET[searchField] = '$_GET[searchString]'  ORDER BY $sidx $sord offset $start limit $limit";
        }
