...
function generarPDFReten($id) {
    conectarse();
    $consulta = pg_query("select * from empresa inner join gastos on empresa.id_empresa  = gastos.id_empresa inner join retencion_fuente_factura_compra on retencion_fuente_factura_compra.id_factura=gastos.id_gastos left join proveedores on gastos.id_proveedor=proveedores.id_proveedor inner join tipo_documento on tipo_documento.id_tdocu=proveedores.id_tdocu where retencion_fuente_factura_compra.id_factura=gastos.id_gastos AND retencion_fuente_factura_compra.id_retencion_fuente_factura_compra= '" . $id . "'");

    while ($row = pg_fetch_row($consulta)) {
        $ruc = $row[2];

        $numeroAutorizacion = $row[74];
        ...
        $obligado = $row[18];
        $contribuyente = $row[78];
        $identificacion = $row[77];
        $direcion = $row[81];
        $telefono = $row[82];
        $email = $row[88];
        $tipoDocumento = $row[26];
        $secuencial = $row[69];
        $ip = $secuencial;
        $iparr = split("\-", $ip);
        $secuencial = $iparr[2];
