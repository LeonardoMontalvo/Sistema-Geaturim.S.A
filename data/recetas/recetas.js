...
function guardar_receta() {
    ...
    if ($("#producto2").val() == "") {
        ...
        if (tam.length == 0) {
            $("#codigo_barras").focus();
            alertify.error("Error... Ingrese productos a la receta");
        } else {
            ...
            $.ajax({
                ...
                data: "producto=" + $("#cod_productos2").val() + "&comprobante=" + $("#comprobante").val() + "&fecha_actual=" + $("#fecha_actual").val() + "&hora_actual=" + $("#hora_actual").val() + "&costo=" + $("#tot").val() + "&campo1=" + string_v1 + "&campo2=" + string_v2 + "&campo3=" + string_v3 + "&campo4=" + string_v4 + "&campo5=" + string_v5 + "&nombre=" + $("#nombre").val(),
                success: function(data) {
                    var val = data;
                    if (val != 0) {
						alertify.alert("Receta Guardada Correctamente", function(){
                            window.open("../../reportes/receta.php?id="+val,'_blank');    
                            location.reload();
                        }); 
                    }else{
                        alertify.alert("El nombre de la receta ya existe");
...
function modificar_receta() {
    ...
    if ($("#producto2").val() == "") {
        ...
        if (tam.length == 0) {
            ...
            $.ajax({
                type: "POST",
                url: "modificar_recetas.php",
                data: "id_receta=" + $("#id_receta").val() + "&producto=" + $("#cod_productos2").val() + "&comprobante=" + $("#comprobante").val() + "&fecha_actual=" + $("#fecha_actual").val() + "&hora_actual=" + $("#hora_actual").val() + "&costo=" + $("#tot").val() + "&campo1=" + string_v1 + "&campo2=" + string_v2 + "&campo3=" + string_v3 + "&campo4=" + string_v4 + "&campo5=" + string_v5 + "&nombre=" + $("#nombre").val(),
                success: function(data) {
                    var val = data;
                    if (val != 0) {
                        alertify.alert("Receta Modificada Correctamente", function(){
                            window.open("../../reportes/receta.php?id="+val,'_blank'); 
                            //myWindow.focus();
                            //myWindow.print();                                                                      
                            location.reload();
...
function flecha_atras() {
    $.ajax({
        ...
        data: "comprobante=" + $("#comprobante").val() + "&tabla=" + "recetas" + "&id_tabla=" + "id_receta" + "&tipo=" + 1,
        success: function(data) {
            var val = data;
            if(val != "") {
                $("#comprobante").val(val);
                ...
                $.getJSON('retornar_receta.php?com=' + valor, function(data) {
                    var tama = data.length;
                    if (tama != 0) {
                        for (var i = 0; i < tama; i = i + 8) {
                            ...
                            $("#tot").val(data[i + 4]);
                            $("#totx").val(parseFloat(data[i + 4]).toFixed(2));
                        }
                    }
                });

                $.getJSON('retornar_receta2.php?com=' + valor, function(data) {
                    var tama = data.length;
                    var descuento = 0;
                    var total = 0;
                    var su = 0;
...
function flecha_siguiente() {
    $.ajax({
        type: "POST",
        url: "../../procesos/flechas.php",
        data: "comprobante=" + $("#comprobante").val() + "&tabla=" + "recetas" + "&id_tabla=" + "id_receta" + "&tipo=" + 2,
        success: function(data) {
            var val = data;
            if(val != "") {
                $("#comprobante").val(val);
...
function aceptar() {
    ...
    $.ajax({
        type: "POST",
        url: "anular_receta.php",
        data: "comprobante=" + $("#id_receta").val() + "&fecha_anulacion=" + $("#fecha_actual").val(),
        success: function(data) {
            var val = data;
            if (val == 1) {
                    alertify.alert("Receta Suspendida Correctamente", function(){
                    location.reload();
                });
            }
        }
...
function activar_receta() {

    $.ajax({
        type: "POST",
        url: "activar_receta.php",
        data: "comprobante=" + $("#id_receta").val() + "&fecha_anulacion=" + $("#fecha_actual").val(),
        success: function(data) {
            var val = data;
            if (val == 1) {
                    alertify.alert("Receta Activada Correctamente", function(){
                    location.reload();
                });
            }
        }
...
function inicio() {
    ...
    $("#btnGuardar").on("click", guardar_receta);
    $("#btnModificar").on("click", modificar_receta);
    $("#btnNuevo").on("click", limpiar_factura);
    $("#btnAnular").on("click", anular_receta);
    $("#btnActivar").on("click", activar_receta);
    ...
    $("#btnImprimir").click(function () {  
        if($("#id_factura_venta").val()==""){
            ...
            window.open("../../reportes/receta.php?id="+$("#id_receta").val(),'_blank');
        }
    });
    ...
    jQuery("#list").jqGrid({
        datatype: "local",
        colNames: ['', 'ID', 'Código', 'Producto', 'P. Costo', 'Cantidad', 'Unidad', 'Unidadx', 'P. Total'],
        colModel: [
        ...
        editoptions:{
            ...
            onclickSubmit: function(rp_ge, rowid) {
                ...
                alertify.alert("cambio");
                var total = $("#totx").val()-ret.total;
                var nuevo=ret.cantidad*ret.precio_u;
                ret.total=nuevo.toFixed(2);
                total=total+nuevo;
        ...
        var ret = jQuery("#list").jqGrid('getRowData', id);
            ...
            if(name == 'cantidad') {
                ...
                jQuery("#list").jqGrid('setRowData',rowid,{total: nuevo.toFixed(2)});
                total=total+nuevo;
   ...
   // buscador facturas ventas  
    jQuery("#list2").jqGrid({
        url: 'xmlBuscarReceta.php',
        datatype: 'xml',
        colNames: ['ID','PRODUCTO', 'COSTO TOTAL','FECHA'],
        colModel: [
            {name: 'id_receta', index: 'id_receta', editable: false, search: false, hidden: false, editrules: {edithidden: false}, align: 'center',frozen: true, width: 50},
