...
$(document).keydown(function(e) {
    ...
    // Guardar Reservación
    if(keycode == 119) { guardar_reservacion()}
...
function guardar_reservacion() {
    ...
    if ($("#ruc_ci").val() == "") {
        ...
        if ($("#nombre_cliente").val() == "") {
            ...
            if (tam.length == 0) {
                $("#codigo_barras").focus();
                alertify.error("Error... Ingrese productos a la reservación");
            } else {
                ...
                $.ajax({
                    ...
                    data: "id_cliente=" + $("#id_cliente").val() + "&comprobante=" + $("#comprobante").val() + "&fecha_actual=" + $("#fecha_actual").val() + "&hora_actual=" + $("#hora_actual").val() + "&tarifa0=" + $("#total_p").val() + "&tarifa12=" + $("#total_p2").val() + "&iva=" + $("#iva").val() + "&tot=" + $("#tot").val() + "&campo1=" + string_v1 + "&campo2=" + string_v2 + "&campo3=" + string_v3 + "&campo4=" + string_v4 + "&campo5=" + string_v5+ "&campo6=" + string_v6,
                    success: function(data) {
                        var val = data;
                        if (val != 0) {
							alertify.alert("Reservación Guardada Correctamente", function(){
                                window.open("../../reportes/reservacion.php?id="+val,'_blank');    
                                location.reload();
                            }); 
                        }
                    }
...
function modificar_reservacion() {
    ...
    if ($("#id_factura_venta").val() == "") {
        alertify.error("Seleccione una reservación");
            ...
            if ($("#nombre_cliente").val() == "") {
                ...
                if (tam.length == 0) {
                    ...
                    $.ajax({
                        ...
                        data: "id_reservacion=" + $("#id_factura_venta").val() + "&id_cliente=" + $("#id_cliente").val() + "&comprobante=" + $("#comprobante").val() + "&fecha_actual=" + $("#fecha_actual").val() + "&hora_actual=" + $("#hora_actual").val() + "&tarifa0=" + $("#total_p").val() + "&tarifa12=" + $("#total_p2").val() + "&iva=" + $("#iva").val() + "&tot=" + $("#tot").val() + "&campo1=" + string_v1 + "&campo2=" + string_v2 + "&campo3=" + string_v3 + "&campo4=" + string_v4 + "&campo5=" + string_v5+ "&campo6=" + string_v6,
                        success: function(data) {
                            var val = data;
                            if (val != 0) {
                                alertify.alert("Reservación Modificada Correctamente", function(){
...
function flecha_atras() {
    $.ajax({
        ...
        data: "comprobante=" + $("#comprobante").val() + "&tabla=" + "reservaciones" + "&id_tabla=" + "id_reservacion" + "&tipo=" + 1,
        success: function(data) {
            var val = data;
            if(val != "") {
                $("#comprobante").val(val);
                ...
                $.getJSON('retornar_reservacion.php?com=' + valor, function(data) {
                    var tama = data.length;
                    if (tama != 0) {
                        for (var i = 0; i < tama; i = i + 13) {
                            ...
                            $("#sub").val(parseFloat(data[i + 9]) + parseFloat(data[i + 10]));
                            $("#iva").val(data[i + 11]);
                            $("#tot").val(data[i + 12]);
                            $("#total_px").val(parseFloat(data[i + 9]).toFixed(2));
                            $("#total_p2x").val(parseFloat(data[i + 10]).toFixed(2));
                            $("#subx").val((parseFloat(data[i + 9]) + parseFloat(data[i + 10])).toFixed(2));
                            $("#ivax").val(parseFloat(data[i + 11]).toFixed(2));
                            $("#totx").val(parseFloat(data[i + 12]).toFixed(2));
                        }
                    }
                });

                $.getJSON('retornar_reservacion2.php?com=' + valor, function(data) {
                    var tama = data.length;
                    var descuento = 0;
                    var total = 0;
                    var su = 0;
...
function flecha_siguiente() {
    $.ajax({
        type: "POST",
        url: "../../procesos/flechas.php",
        data: "comprobante=" + $("#comprobante").val() + "&tabla=" + "reservaciones" + "&id_tabla=" + "id_reservacion" + "&tipo=" + 2,
        success: function(data) {
            var val = data;
            if(val != "") {
                $("#comprobante").val(val);
...
function aceptar() {
    if($("#motivo").val()==""){
        alertify.alert("Por favor ingrese un motivo");
    }else{
        var v1 = new Array();
        var v2 = new Array();
        var string_v1 = "";
        ...
        $.ajax({
            type: "POST",
            url: "anular_reservacion.php",
            data: "comprobante=" + $("#comprobante").val() + "&campo1=" + string_v1 + "&campo2=" + string_v2+ "&fecha_anulacion=" + $("#fecha_actual").val()+"&motivo="+$("#motivo").val(),
            success: function(data) {
                var val = data;
                if (val == 1) {
                    alertify.alert("Reservación Suspendida Correctamente", function(){
                        location.reload();
                    });
                }
            }
...
function inicio() {
    ...
    $("#btnGuardar").on("click", guardar_reservacion);
    $("#btnModificar").on("click", modificar_reservacion);
    ...
    $("#btnImprimir").click(function () {  
        if($("#id_factura_venta").val()==""){
            alertify.alert("Reservación no Creada");
        }else{
            window.open("../../reportes/reservacion.php?id="+$("#id_factura_venta").val(),'_blank');
        }
    });
    ...
    jQuery("#list").jqGrid({
        datatype: "local",
        colNames: ['', 'ID', 'Código', 'Producto', 'Cantidad', 'PVPx', 'Descuentox','Calculadox', 'Totalx', 'PVP', 'Descuento','Calculado', 'Total', 'Iva', 'Pendientes','Incluye'],
        colModel: [
            ...
            {name: 'precio_u', index: 'precio_u', hidden: true, editable: false, search: false, frozen: true, editrules: {required: true}, align: 'center', width: 110, editoptions:{maxlength: 10, size:15,dataInit: function(elem){$(elem).bind("keypress", function(e) {return punto(e)})}}}, 
            ...
            {name: 'precio_ux', index: 'precio_ux', editable: false, search: false, frozen: true, editrules: {required: true}, align: 'center', width: 110, editoptions:{maxlength: 10, size:15,dataInit: function(elem){$(elem).bind("keypress", function(e) {return punto(e)})}}}, 
   ...
   // buscador facturas ventas  
    jQuery("#list2").jqGrid({
        url: 'xmlBuscarReservacion.php',
        datatype: 'xml',
        colNames: ['ID','IDENTIFICACIÓN','CLIENTE', 'MONTO TOTAL','FECHA'],
        colModel: [
            {name: 'id_reservacion', index: 'id_reservacion', editable: false, search: false, hidden: false, editrules: {edithidden: false}, align: 'center',frozen: true, width: 50},
