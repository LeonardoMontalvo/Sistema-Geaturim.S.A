...
function guardar_vendedores() {
    ...
    if ($("#ruc_ci").val() === "") {
        ...
        if ($("#nombre").val() === "") {
            ...
            if ($("#direccion_vend").val() === "") {
                $("#direccion_vend").focus();
                ...
                if ($("#nro_celular").val() === "") {
                    ...
                    $.ajax({
                        ...
                        data: "&nombre=" + $("#nombre").val() +
                                ...
                                "&correo=" + $("#correo").val() +
                                "&direccion_vend=" + $("#direccion_vend").val(),
...
function aceptar() {
    $.ajax({
        type: "POST",
        url: "eliminar_vendedores.php",
        data: "ruc_ci=" + $("#ruc_ci").val(),
        success: function (data) {
            var val = data;
            if (val == 1) {
                alertify.alert(' El Vendedor esta asignado a una ruta');
                setTimeout(function () {
                    location.reload();
                }, 1000);
            } else if(val == 2){
              alertify.alert(' Vendedor esta asignado a una ruta y a un cliente');
                setTimeout(function () {
                    location.reload();
                }, 1000);
...
//            else if(val == 3){
//              alertify.alert('El Vendedor esta asignado a una ruta , a un cliente y a facturas');
////                setTimeout(function () {
////                    location.reload();
////                }, 1000);
//            }
...
function inicio() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (val == 1) {
                    $("#ruc_ci").val("");
                    $("#ruc_ci").focus();
                    alertify.error("Error... El vendedor ya ésta registrado");
                } else {
    ...
    $("#btnGuardar").on("click", guardar_vendedores);
    $("#btnModificar").on("click", modificar_vendedores);
    $("#btnEliminar").on("click", eliminar_vendedores);
    $("#btnNuevo").on("click", nuevo_vendedores);
    ...
    jQuery("#list").jqGrid({
        url: 'datos_vendedores.php',
        datatype: 'xml',
        colNames: ['Codigo', 'Nombres', 'Cédula', 'Teléfono', 'Movil', 'Correo', 'Dirección'],
        colModel: [
            {name: 'id_vendedor', index: 'id_vendedor', editable: true, align: 'center', width: '30', search: false, frozen: true, editoptions: {readonly: 'readonly'}, formoptions: {elmprefix: ""}},
            {name: 'nombre', index: 'nombre', editable: true, align: 'left', width: '120', search: true, frozen: true, editoptions: {readonly: 'readonly'}, formoptions: {elmprefix: ""}},
            ...
            {name: 'direccion_vend', index: 'direccion_vend', editable: true, align: 'center', width: '120', search: false, frozen: false, editoptions: {readonly: 'readonly'}, formoptions: {elmprefix: ""}}
        ...
        ondblClickRow: function () {
            ...
            jQuery("#list").jqGrid('GridToForm', id, "#vendedores_form");
            $("#btnGuardar").attr("disabled", true);
