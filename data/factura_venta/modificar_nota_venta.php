...
$stok_ant_cantidad=0;
$stok_ant_um=0;
   $consulta_prod = pg_query("SELECT cantidad, cantidad_unidad FROM detalle_facturas_novalidas where cod_productos='$arreglo1[$i]' and id_facturas_novalidas=$cont1 and  estado='Activo'");
...
//    foreach (obtenerDetallaNota($_POST['comprobante'], $conpuntoresult) as $item) {
        ...
        $stock = obtenerStock($arreglo1[$i], $conpuntoresult);
        $total = number_format(($cantidad * $arreglo3[$i]), 4, '.', '');
...
//        updateKardexValorizado($_POST['comprobante'], $conpuntoresult, $item['cod_productos'], 'Inactivo', 'NV');
        $costoPromedio = obtenerCostoPromedioUnitarioAnular($arreglo1[$i], $conpuntoresult, $_POST['comprobante'], 'DNV');
        procesarKardexEntrada($arreglo1[$i], $documento, $cantidad, $stock, $costoPromedio, 'Activo', $conpuntoresult, 'DNV', $_POST['comprobante'], $total, NULL, NULL, '', NULL, NULL, $cliente1, $_SESSION['id']);
//    }
