...
function getPagoV($idfactura)
{
    $sql = "SELECT  id_pagos_venta,monto_credito,saldo FROM pagos_venta where  estado='Activo' and id_factura_venta='$idfactura' and tipo_documento='Factura'";
    $res = pg_query($sql);
    $rows = pg_fetch_all($res);
    if (empty($rows)) {
        return [];
