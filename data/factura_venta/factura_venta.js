                            ...
                            if ($("#cantidad_unidad").val() != "") {
                                ...
                                if (parseInt($("#cantidad").val()) > parseInt($("#disponibles").val())) {
                                    ...
                                    if (filas.length == 0) {
                                        ...
                                        console.log("entroo aki" + $("#venta_iva_1").val());
                                        var datarow = {
                                            id_list: item1,
                                            cod_producto: $("#cod_producto").val(),
                                            codigo: $("#codigo").val(),
                                ...
                                if (
                                    ...
                                    if (filas2.length == 0) {
                                        ...
                                        if ($("#tipo_venta").val() == "FACTURA") {
                                            ...
                                            if ($("#id_nota_venta").val() != "") {
                                                id_factura_nota = $("#id_nota_venta").val();
                                            } else {
...
//
function modificar_nv() {

    var filas2 = jQuery("#listPagoreten_mixto").jqGrid("getRowData");
    if (filas2.length != 0 && $("#valor_factura").val() == "") {
        alertify.error("Error... la Nota de venta ya tiene registrado formas de pago, no se puede motificar");
    } else {

        var tam = jQuery("#list").jqGrid("getRowData");
        if ($("#id_nota_venta").val() == "") {
            alertify.error("Seleccione una Nota Venta");
                                            ...
                                            success: function (data) {
                                                ...
                                                if ($("#tipo_venta").val() == "NOTA") {
                                                    if (val != 0) {
                                                        ...
                                                        alertify.confirm("Modificado Correctamente ¿Desea cambiar a Factura..? ",
                                                            function (e) {
                                                                if (e) {
                                                                    actualizar_compro();
                                                                } else {
                                                                    ...
                                                                    alertify.alert(
                                                                        "Nota Venta Modificada correctamente",
                                                                        function () {
                                                                            var myWindow = window.open(
                                                                                formatoNotaVenta + "?hoja=A4&id=" +
                                                                                val,
...
function ingresar_cambio(e) {
    ...
    if ($("#totx").val() > 500.000 && $("#observacionPago").val() == "") {
    ...
    if ($("#cod_producto").val() !== "") {
        ...
        $.getJSON(
            ...
            (data) => {
                ...
                $("#precio").val(numFormatter(2).format(cantidadu * precioc));
...
function cambioForma() {
    ...
    var x = document.getElementById("formas").selectedIndex;
    //alertify.alert(x+"-");
    var porNombre = document.getElementById("formas")[x].value;
...
function actualizar_compro() {
    ...
    $("#btnActualizartrans_g").attr("disabled", true);

    $("#tipo_ven_factura").prop("selected", true);
    ...
    $("#venta_iva_1").attr("disabled", true);
    ...
    if ($("#num_factura").val() == "") {
        ...
        var str1 = $("#comprobante_temp").val();
        var res1 = parseInt(str1);
        res1 = res1;
        $("#comprobante").val(res1);
...
function cobroTarjeta() {
    ...
    var iva_pventa = 0;
    var subtotal_total = 0;
    if (document.getElementById('cobro_tarjeta').checked == true) {
        ...
        for (var t = 0; t < fil.length; t++) {
            ...
            total = parseFloat(multi);
            if (dd["iva"] == 'Si') {
                pendi = numFormatter(4).format((total + calcularIva(Number(total), Number(dd['tarifa']))));
            } else {
            ...
            jQuery("#list").jqGrid("setRowData", id_mix, {
                ...
                total: numFormatter(4).format(total),
                pendiente: pendi,
...
//            subtotal0 = parseFloat(subtotal0);
//            subtotal12 = parseFloat(subtotal12);
//            console.log("subtotal12", subtotal12);
//            subtotal_total = parseFloat(subtotal_total);
//            iva12 = parseFloat(iva12);
//            descu_total = parseFloat(descu_total);
//        } else {
...
function inicio() {
    ...
    $("#btnActualizartrans_g").on("click", actualizar_compro);

    $("#formas").on("change", cambioForma);
    ...
    $("#cantidad").keyup(function () {
        ...
        //TODO comentado para permitir seleccionar precio de select tipo de precio
        /*if ($("#cantidad_unidad").val() == "") {
         $.ajax({
         type: "POST",
         ...
         $.ajax({
         type: "POST",
         url: "buscar_cant_mayo_nego_um.php",
         data: "id_prod=" + $("#cod_producto").val() +
         "&unidad_medida=" +
         $("#unidad_medida").val(),
         success: function (data) {
         var val = data;
    ...
    $("#btnModificarnv").on("click", modificar_nv);
    ...
    function mayorista() {
        //TODO retornar para permitir seleccionar precio de select tipo de precio
        return;
        console.log("mayoristaa");
                    ...
                    function (data) {
                        ...
                        if (tama != 0) {
                            for (var i = 0; i < tama; i = i + 15) {
                                ...
                                $("#cod_producto").val(data[i + 6]).change();
                                $("#des").val(data[i + 7]);
    ...
    function barras() {
        ...
        var cod = $("#codigo_barras").val();
        $.getJSON(
            "search_cod_producto.php?codigo_barras=" +
            codigo +
            "&precio=" +
            precio +
            "&cod=" +
            ...
            function (data) {
                ...
                if (tama != 0) {
                    for (var i = 0; i < tama; i = i + 1) {
                        ...
                        $("#cod_producto").val(data[i]).change();
    ...
    // fin
    // accion limpiar
    $("#tipo_precio").change(function (e) {
        obtenerPrecioProducto(e.target.value);
        return false;
        $("#codigo_barras").val("");
        $("#cod_producto").val("").change();
        $("#codigo").val("");
    ...
    jQuery("#list").jqGrid({
        ...
        colModel: [
            ...
            { name: "tarifa", index: "tarifa", hidden: true, }, // ES TRUE
            { name: "valor_iva", index: "valor_iva", hidden: true, }, // ES TRUE
            { name: "cod_impuesto", index: "cod_impuesto", hidden: true, }, // ES TRUE
            { name: "cod_tarifa", index: "cod_tarifa", hidden: true, }, // ES TRUE
            {
            ...
            if (name == "precio_ux_iva") {
                ...
                if (descuento_grid != "0") {
                    ...
                    jQuery("#list").jqGrid("setRowData", rowid, {
                        ...
                        total: total,
                        pendiente: numFormatter(2).format(result),
                        precio_u: numFormatter(2).format(precio),
        ...
        .jqGrid({
            ...
            ondblClickRow: function () {
                ...
                if (id) {
                    ...
                    // agregregar datos factura
                    $("#comprobante").val(valor);
                    $("#comprobante_antnv").val(valor);
                    $("#btnGuardar").attr("disabled", true);
                    $("#btnModificarnv").attr("disabled", false);
                        ...
                        if (tama != 0) {
                            ...
                            for (var i = 0; i < tama; i = i + 18) {
                                ...
                                if (data[i + 11] == "Pasivo") {
                                    ...
                                    if (data[i + 11] == "Factura") {
                                        $('#btnActualizar_fac').hide();
                                        $("#estado").append($("<h3>").text("Cambio a Factura"));
                                        $("#estado h3").css("color", "red");
                                        $("#btnAnular").attr("disabled", "disabled");
                                    } else {
                                        $('#btnActualizar_fac').show();
                    ...
                    $.getJSON("retornar_nota_venta2.php?com=" + valor,
                        function (data) {
                            var tama = data.length;
                            var descuento = 0;
                            var total = 0;
                            ...
                            if (tama !== 0) {
                                for (var i = 0; i < tama; i = i + 18) {
                                    ...
                                    var datarow = {
                                        ...
                                        total: total,
                                        precio_ux: precio.toFixed(3),
                                        descuentox: parseFloat(desc).toFixed(3),
                                        cal_desx: resultado.toFixed(3),
                                        totalx: total.toFixed(3),
                                        iva: data[i + 7],
                            ...
                            calcularTotalesTablaProductos(true);
                        });
                    // fin
                    $.getJSON("retornar_mixto_grid_nota.php?com=" + valor,
                        function (data) {
                            ...
                            var tama = data.length;
                            if (tama != 0) {
                                $("#formaspago").attr("disabled", "disabled");
                                for (var i = 0; i < tama; i = i + 5) {
                                    var datarow = {
                                        forma_pago_mixto: data[i],
                                        tarjeta_credito: data[i + 1],
                    ...
                    $.getJSON("retornar_proforma.php?id2=" + valor, function (data) {
                        ...
                        if (tama === 0) {
                            ...
                            for (var i = 0; i < tama; i = i + 17) {
                                ...
                                let total_sin_dcto = multi;
                                let preciosi = Number(precio);
                                let precioci = preciosi * (1 + (Number(data[i + 12]) / 100));
                                ...
                                var datarow = {
                                    ...
                                    valor_iva: data[i + 13],
                                    tarifa: data[i + 12],
                                    cod_impuesto: data[i + 14],
                                    cod_tarifa: data[i + 15],
                                    totalx_sin_descu: total_sin_dcto,
                                    pvp_iva_descuento: obtenerPvp_u(precioci, desc),
                                    precio_ux_iva: obtenerPvp_u_sin_descu(precioci, desc),
                                    pvp_iva_sin_descuento: obtenerPvp_u_sin_descu(precioci, desc),
                                    precio_unitario_descuento: obtenerPvp_u_con_descu(precio, desc),
                                    precio_unitario_descuento_x: obtenerPvp_u_con_descu_sin_r(precio, desc),
                                    pvp_iva_descuento_t: obtenerPvp_u_t(precioci, desc, temp),
                                };
                                var su = jQuery("#list").jqGrid("addRowData", data[i], datarow);
                                var ivas = data[i + 8];
                            }
...
function obtenerPvp_u(pvpiva = null, descuento = null) {
    var descuento_iva = 0;
    var precio_iva = 0;
...
function obtenerPvp_u_sin_descu(pvpiva = null, descuento = null) {
    var descuento_iva = 0;
    var precio_iva = 0;
    var resultado_pvp_iva = 0;
    var total_iva_descu = 0;
...
function obtenerPvp_u_con_descu(pvp = null, descuento = null) {
    var descuento_iva = 0;
    var precio_iva = 0;
    var resultado_pvp_iva = 0;
    var total_iva_descu = 0;
...
function obtenerPvp_u_con_descu_sin_r(pvp = null, descuento = null) {
    var descuento_iva = 0;
    var precio_iva = 0;
    var resultado_pvp_iva = 0;
    var total_iva_descu = 0;
...
function obtenerPvp_u_t(pvpiva = null, descuento = null, cantidad = null) {
    var multi_t = 0;
    ...
    precio_iva_t = parseFloat($("#venta_iva_1").val());
    if (pvpiva) {
        precio_iva_t = Number(pvpiva);
    }
    multi_t = cant * parseFloat(precio_iva_t);
    desc = $("#descuento").val();
    if (descuento) {
        desc = Number(descuento);
    }
    ...
    if (desc != "") {
        ...
        total_iva_descu_t = multi_t - resultado_pvp_iva_t;
        total_iva_descu_t = total_iva_descu_t.toFixed(2);
        return total_iva_descu_t;
    } else {
        total_iva_descu_t = multi_t
        total_iva_descu_t = total_iva_descu_t.toFixed(2);
        return total_iva_descu_t;
    }
...
function obtenerPvp_u_t_grid(venta_iva_1, cantidad, descuento, iva, valor_iva_j) {
    ...
    var desc = 0;
    var precioci = 0;
    multi_t = cantidad * parseFloat(venta_iva_1);

    if (iva == 'Si') {
        precioci = parseFloat(multi_t) * (1 + (valor_iva_j / 100));
        console.log("ivasi1/" + multi_t);
    } else {
        precioci = multi_t;
        console.log("ivasi2" + precioci);
    }
    precio_iva_t = parseFloat(precioci);
    ...
    if (descuento != "0") {
        ...
        resultado_pvp_iva_t = Math.round(descuento_iva_t * Math.pow(10, 2)) / Math.pow(10, 2);
        total_iva_descu_t = precio_iva_t - resultado_pvp_iva_t;
        total_iva_descu_t = Math.round(total_iva_descu_t * Math.pow(10, 2)) / Math.pow(10, 2);
        return total_iva_descu_t;
    } else {
        console.log("des2_/");
        total_iva_descu_t = precio_iva_t;
...
function obtenerPvp_u_sin_descu_gridd(venta_iva_1, cantidad, descuento, iva, valor_iva_j) {
    ...
    return Math.round(precioci * Math.pow(10, 2)) / Math.pow(10, 2);
    ;
...
async function cargarFacturaDblclick(id) {
    if (id) {
        ...
        await $.getJSON("retornar_factura_venta.php?com=" + valor, function (data) {
            var tama = data.length;
            t = data[23];
            if (tama !== 0) {
        ...
        $("#formaspago_mixto_reten").val("cxc").change();
        $("#total_retencion").val("");
...
async function buscarIva(codprod) {
    try {
        const res = await $.ajax({
            type: "POST", url: "buscar_iva.php",
            data: { "id_producto": codprod },
            dataType: "json",
        });
        //TODO descomentar para controlar tarifas antes del 2024-04-01
        /* if (res.valor > 0) {
         if (new Date($("#fecha_actual").val()) < new Date('2024-04-01')) {
         res.valor = 12;
         res.codigo_timpu = 2;
         res.codigo_taimpuesto = 2;
         }
         } */
        calculoIVA = Number(res.valor);
        codImpuesto = Number(res.codigo_timpu);
        codTarifa = Number(res.codigo_taimpuesto);
    } catch (error) {
        limpiarInfoIVA();
    }
...
function calcularTotalesTablaProductos(buesqueda = false) {
    ...
    console.log("tabla pro2");
    $("#items").val(rows.length);
    $("#tot").val(total);
    ...
    $("#descxax").val(descuentoprods.toFixed(2));
    envdescprod = Number(descuentoprods.toFixed(2));
    $("#desctotal").val(envdescprod + envdescfact);
...
function obtenerPrecioProducto(tipoprecio) {
    $.ajax({
        url: "obtener_producto.php",
        method: "GET",
        dataType: "json",
        data: { id_producto: $("#cod_producto").val() },
        success: function (data) {
            $("#venta_iva_1").val("");
            if (tipoprecio == "MINORISTA") {
                $("#p_venta").val(data.iva_minorista)
            }
            if (tipoprecio == "MAYORISTA") {
                $("#p_venta").val(data.iva_mayorista)
            }
